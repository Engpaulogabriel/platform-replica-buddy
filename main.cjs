...
// ─────────────────────────────────────────────────────────────
const COM_WATCHDOG_CHECK_MS = 30_000;
const COM_WATCHDOG_THRESHOLD_MS = 120_000;
let comAlertSent = false;

async function sendComWatchdogNotify(message) {
  try {
    if (!supabase || !farmId) {
      pushLog("warn", "system", `[WATCHDOG] contexto ausente — não notifica: ${message}`);
      return;
    }
    ...
    try { pushLog("warn", "system", `[WATCHDOG] erro ao notificar: ${e && e.message || e}`); } catch (_) {}
  }
}

function comWatchdogTick() {
  try {
    if (!bridgeReady) return; // bridge fechada = outro problema
    if (!lastRxTimestamp) return; // ainda não houve nenhum RX inicial
    const elapsed = Date.now() - lastRxTimestamp;
    if (elapsed > COM_WATCHDOG_THRESHOLD_MS && !comAlertSent) {
      comAlertSent = true;
      pushLog("warn", "serial", `[WATCHDOG] Sem RX há ${Math.round(elapsed/1000)}s — alerta enviado`);
      console.log("[WATCHDOG] Sem RX há 2 min — alerta enviado");
      void sendComWatchdogNotify("⚠️ Sem comunicação serial há 2 minutos — verificar cabo/adaptador USB-Serial e porta COM");
    }
  } catch (e) {
    try { pushLog("warn", "system", `[WATCHDOG] com tick exception: ${e && e.message || e}`); } catch (_) {}
  }
}
setInterval(comWatchdogTick, COM_WATCHDOG_CHECK_MS);

function notifyComRecoveryIfNeeded() {
  if (!comAlertSent) return;
  comAlertSent = false;
  pushLog("info", "serial", "[WATCHDOG] Comunicação restabelecida");
  console.log("[WATCHDOG] Comunicação restabelecida");
  void sendComWatchdogNotify("✅ Comunicação serial restabelecida após interrupção");
}

// ─── v3.19.0 — Watchdog de alertas técnicos com recuperação pareada ─────────
// Cada tipo de problema tem flag persistente (bridge/com/pumps/agent).
// Só envia recuperação se o alerta de problema tiver sido enviado antes.
// Alertas vão para whatsapp-automation-notify com source="electron_watchdog"
// (edge function encaminha apenas para admins RENOV).
const WATCHDOG_STATE_FILE = (() => {
  try {
    const dir = (app && app.getPath) ? app.getPath("userData")
      : path.join(os.homedir(), "AppData", "Roaming", "GestorDeBombasKey");
    try { fs.mkdirSync(dir, { recursive: true }); } catch (_) {}
    return path.join(dir, "watchdog_state.json");
  } catch (_) { return null; }
})();
let _wdState = null;
function wdLoad() {
  if (_wdState) return _wdState;
  try {
    if (WATCHDOG_STATE_FILE && fs.existsSync(WATCHDOG_STATE_FILE)) {
      _wdState = JSON.parse(fs.readFileSync(WATCHDOG_STATE_FILE, "utf8"));
    }
  } catch (_) {}
  if (!_wdState || typeof _wdState !== "object") _wdState = {};
  for (const k of ["bridge", "com", "pumps", "agent"]) {
    if (typeof _wdState[k] !== "boolean") _wdState[k] = false;
  }
  return _wdState;
}
function wdPersist() {
  try {
    if (WATCHDOG_STATE_FILE && _wdState) fs.writeFileSync(WATCHDOG_STATE_FILE, JSON.stringify(_wdState));
  } catch (_) {}
}
let _farmNameCache = null;
async function getFarmName() {
  if (_farmNameCache) return _farmNameCache;
  try {
    if (!supabase || !farmId) return "";
    const { data } = await supabase.from("farms").select("name").eq("id", farmId).maybeSingle();
    _farmNameCache = (data && data.name) ? String(data.name) : "";
  } catch (_) { _farmNameCache = ""; }
  return _farmNameCache;
}
async function _wdSend(message) {
  try {
    if (!supabase || !farmId) {
      pushLog("warn", "system", `[WATCHDOG] contexto ausente — não notifica: ${message}`);
      return;
...
// Envia alerta de problema. Só dispara se ainda não foi enviado (idempotente).
async function wdAlert(kind, message) {
  const st = wdLoad();
  if (st[kind]) return; // já enviado — não duplica
  st[kind] = true;
  wdPersist();
  pushLog("warn", "watchdog", `[WATCHDOG] alerta ${kind}: ${message}`);
  await _wdSend(message);
}
// Envia recuperação — SOMENTE se o alerta de problema foi enviado.
async function wdRecover(kind, message) {
  const st = wdLoad();
  if (!st[kind]) return; // problema nunca alertado — não spamar recuperação
  st[kind] = false;
  wdPersist();
  pushLog("info", "watchdog", `[WATCHDOG] recuperação ${kind}: ${message}`);
  await _wdSend(message);
}

// ─── Bridge down / recovered ────────────────────────────────────────────────
// Bridge caída por mais de BRIDGE_DOWN_ALERT_MS dispara alerta.
const BRIDGE_DOWN_ALERT_MS = 60_000;
let _bridgeDownSince = null;
let _bridgeDownTimer = null;
function markBridgeDown(reason) {
  if (_bridgeDownSince) return;
  _bridgeDownSince = Date.now();
  if (_bridgeDownTimer) clearTimeout(_bridgeDownTimer);
  _bridgeDownTimer = setTimeout(async () => {
    if (!bridgeReady && _bridgeDownSince) {
      const name = await getFarmName();
      const suffix = name ? ` — ${name}` : "";
      void wdAlert("bridge", `⚠️ Bridge serial CAIU${suffix} — sem comunicação com PLCs${reason ? ` (${reason})` : ""}`);
    }
  }, BRIDGE_DOWN_ALERT_MS);
}
async function markBridgeUp() {
  if (_bridgeDownTimer) { clearTimeout(_bridgeDownTimer); _bridgeDownTimer = null; }
  _bridgeDownSince = null;
  const name = await getFarmName();
  const suffix = name ? ` — ${name}` : "";
  void wdRecover("bridge", `✅ Bridge serial restabelecida${suffix} — comunicação normalizada`);
}

// ─── COM não encontrada / encontrada ────────────────────────────────────────
async function wdAlertComNotFound() {
  const name = await getFarmName();
  const suffix = name ? ` — ${name}` : "";
  void wdAlert("com", `⚠️ Porta COM não encontrada${suffix} — nenhuma porta respondeu com frame RENOV. Verificar cabo/adaptador USB-Serial.`);
}
async function wdRecoverCom(port) {
  const name = await getFarmName();
  const suffix = name ? ` — ${name}` : "";
  const portStr = port ? ` (${port})` : "";
  void wdRecover("com", `✅ Porta COM encontrada${portStr}${suffix} — comunicação restabelecida`);
}

// ─── Poços offline (≥50% sem RX >5min) ──────────────────────────────────────
const PUMPS_OFFLINE_CHECK_MS = 60_000;
const PUMPS_OFFLINE_STALE_MS = 5 * 60_000;
const PUMPS_OFFLINE_HIGH = 0.5;   // dispara ≥50%
const PUMPS_OFFLINE_LOW  = 0.25;  // recupera <25%
let _pumpsAlertedCount = 0;
async function pumpsWatchdogTick() {
  try {
    if (!supabase || !farmId) return;
    const { data, error } = await supabase
      ...
      .eq("farm_id", farmId);
    if (error || !Array.isArray(data) || data.length < 4) return;
    const now = Date.now();
    const total = data.length;
    let stale = 0;
    for (const e of data) {
      const t = e.last_communication ? new Date(e.last_communication).getTime() : 0;
      if (!t || (now - t) > PUMPS_OFFLINE_STALE_MS) stale++;
    }
    const ratio = stale / total;
    const st = wdLoad();
    const name = await getFarmName();
    const suffix = name ? ` — ${name}` : "";
    if (!st.pumps && ratio >= PUMPS_OFFLINE_HIGH) {
      _pumpsAlertedCount = stale;
      void wdAlert("pumps", `⚠️ ${stale} de ${total} poços offline (${Math.round(ratio*100)}%)${suffix} — sem resposta há >5min`);
    } else if (st.pumps && ratio < PUMPS_OFFLINE_LOW) {
      const back = Math.max(_pumpsAlertedCount - stale, total - stale);
      void wdRecover("pumps", `✅ Comunicação restabelecida com ${back} poços${suffix}`);
      _pumpsAlertedCount = 0;
    }
  } catch (_) {}
}
setInterval(pumpsWatchdogTick, PUMPS_OFFLINE_CHECK_MS);

// ─── Agente reconectado (envia na inicialização, se havia flag pendente) ────
async function wdAgentRecoveredIfPending() {
  const st = wdLoad();
  if (!st.agent) return; // nunca alertado — nada a fazer
  const name = await getFarmName();
  const suffix = name ? ` — ${name}` : "";
  void wdRecover("agent", `✅ Agente reconectado${suffix} — sistema operacional`);
}
...
const AGENT_VERSION = "3.21.0";

// ─── v3.18.0 — Persistência da última porta COM válida + auto-discovery ────
// Salva a porta que produziu o PRIMEIRO frame RX válido (telemetria ou CFG).
// Na próxima inicialização, prioriza essa porta sobre a config do banco.
// Se nenhuma porta conhecida funcionar, enumera as COMs do sistema e testa cada
// uma até achar o rádio RENOV (a que responde com frame válido).
const LAST_WORKING_COM_FILE = (() => {
  try {
    const dir = (app && app.getPath) ? app.getPath("userData")
      : path.join(os.homedir(), "AppData", "Roaming", "GestorDeBombasKey");
    try { fs.mkdirSync(dir, { recursive: true }); } catch (_) {}
    return path.join(dir, "last_working_com.txt");
  } catch (_) { return null; }
})();
let pendingPortConfirmation = null; // porta abrindo, ainda sem RX válido confirmado
let firstRxSavedForPort = null;     // porta já confirmada nesta sessão (evita re-escrita)
// v3.20.0 — controla cancelamento de retries stale ao conectar
let bridgeConnected = false;
let discoveryRetryTimer = null;
function readLastWorkingCom() {
  try {
    if (!LAST_WORKING_COM_FILE || !fs.existsSync(LAST_WORKING_COM_FILE)) return null;
    const v = fs.readFileSync(LAST_WORKING_COM_FILE, "utf8").trim();
    return v && /^COM\d+$/i.test(v) ? v.toUpperCase() : null;
  } catch (_) { return null; }
}
function saveLastWorkingCom(port) {
  try {
    if (!LAST_WORKING_COM_FILE || !port) return;
    fs.writeFileSync(LAST_WORKING_COM_FILE, String(port).toUpperCase() + "\n", "utf8");
  } catch (_) {}
}

const LOG_RETENTION_DAYS = 7;
const LOG_FILE_MAX_BYTES = 50 * 1024 * 1024; // 50MB por arquivo
// v3.14.1 — Log criptografado AES-256-GCM
const RLOG_UPLOAD_INTERVAL_MS = 5 * 60 * 1000; // 5 min
let logEncryptionKey = null;   // Buffer(32) — apenas em memória
let logKeyLoadInFlight = false;
let rlogUploadTimer = null;
let lastBridgeStateWasOk = true; // p/ detectar transições p/ mensagem de tela
...
const RECENT_MANUAL_RESET_BLOCK_MS = 60_000;
const recentManualByEquipment = new Map(); // equipment_id -> { at, cmdId, tsnn }
const recentManualByTsnn = new Map();      // TSNN -> { at, cmdId, equipmentId }
...
function noteRecentManualCommand(cmd, tsnn) {
  if (!cmd || cmd.type !== "manual" || (cmd.priority ?? 5) <= 0 || isBackendResetCommand(cmd, cmd.frame)) return;
  const at = Date.now();
  const equipmentId = cmd.equipment_id ? String(cmd.equipment_id) : null;
  const normalizedTsnn = tsnn || extractCommandTsnn(cmd.frame || "") || cmd.plc_hw_id || null;
  if (equipmentId) {
    recentManualByEquipment.set(equipmentId, { at, cmdId: String(cmd.id || ""), tsnn: normalizedTsnn ? String(normalizedTsnn) : null });
  }
  if (normalizedTsnn) {
    recentManualByTsnn.set(String(normalizedTsnn), { at, cmdId: String(cmd.id || ""), equipmentId });
  }
}

function getRecentManualForReset(cmd, tsnn) {
  const now = Date.now();
  const equipmentId = cmd?.equipment_id ? String(cmd.equipment_id) : null;
  if (equipmentId) {
    const byEq = recentManualByEquipment.get(equipmentId);
    if (byEq && now - byEq.at < RECENT_MANUAL_RESET_BLOCK_MS) return byEq;
    if (byEq) recentManualByEquipment.delete(equipmentId);
  }
  if (tsnn) {
    const byTsnn = recentManualByTsnn.get(String(tsnn));
    if (byTsnn && now - byTsnn.at < RECENT_MANUAL_RESET_BLOCK_MS) return byTsnn;
    if (byTsnn) recentManualByTsnn.delete(String(tsnn));
  }
  if (
    inflightCmd &&
    inflightCmd.type === "manual" &&
    (inflightCmd.priority ?? 5) > 0 &&
    (!tsnn || inflightTsnn === tsnn) &&
    (now - new Date(inflightCmd.sent_at || inflightCmd.created_at || now).getTime()) < RECENT_MANUAL_RESET_BLOCK_MS
  ) {
    return { at: now, cmdId: String(inflightCmd.id || ""), equipmentId: inflightCmd.equipment_id ? String(inflightCmd.equipment_id) : null, tsnn };
  }
  return null;
}

async function cancelResetBlockedByManual(cmd, tsnn, reason) {
  try {
    await supabase
      .from("commands")
      .update({
        ...
        error_message: reason || "RESET ignorado: reforco manual ativo para este equipamento",
      })
      .eq("id", cmd.id)
      .eq("status", "pending");
  } catch (e) {
    pushLog("warn", "system", `Falha ao cancelar RESET ${String(cmd?.id || "").substring(0,8)} ignorado: ${e.message}`);
  }
  pushLog("warn", "system", `RESET ${String(cmd?.id || "").substring(0,8)} ignorado — reforço manual ativo para TSNN ${tsnn || "?"}`);
}

async function shouldIgnoreResetDueManualReinforcement(cmd, tsnn) {
  if (!cmd || (cmd.priority ?? 5) !== 0) return false;
  const activeReinforcement = getActiveReinforcementForTsnn(tsnn);
  const recentManual = getRecentManualForReset(cmd, tsnn);
  if (!activeReinforcement && !recentManual) return false;
  await cancelResetBlockedByManual(
    cmd,
    tsnn,
    activeReinforcement
      ? "RESET ignorado: reforco TX manual ativo para este TSNN"
      : "RESET ignorado: comando manual recente recebido ha menos de 60s",
  );
  return true;
}
...
async function markCfgCommandExecuted(cmd, frame, cfgLabel, confirmedSetId) {
  ...
  try {
    pushLog("info", "cloud", `INF [CFG] Tentando UPDATE commands id=${cmd.id} status=executed`);
    const { data, error, count, status, statusText } = await withCloudTimeout(
      supabase
        .from("commands")
        .update({
          status: "executed",
    ...
    if (error) {
      pushLog("error", "cloud", `ERR [CFG] UPDATE falhou: ${JSON.stringify({ message: error.message, code: error.code, details: error.details, hint: error.hint, status, statusText })}`);
      throw error;
    }
    pushLog("info", "cloud", `INF [CFG] UPDATE resultado: ${JSON.stringify({ count: typeof count === "number" ? count : null, rows: Array.isArray(data) ? data.length : null, error: null, status, statusText })}`);
    if (!Array.isArray(data) || data.length === 0) {
      pushLog("error", "cloud", `[CFG UPDATE] linha NAO encontrada para id=${cmd.id} (count=${typeof count === "number" ? count : "null"}; provável RLS bloqueando, UUID errado ou registro removido)`);
      throw new Error(`UPDATE commands não alterou linhas para id=${cmd.id}`);
    }
    pushLog("info", "system", `[CFG UPDATE OK] cmd ${cmd.id.substring(0, 8)} -> executed (CFG ${cfgLabel}) response_len=${(frame||"").length}`, null, null);
    if (confirmedSetId) await syncConfirmedSetId(cmd, confirmedSetId);
  } catch (e) {
    pushLog("error", "cloud", `[CFG UPDATE FAIL] id=${String(cmd.id).substring(0, 8)}: ${e.message || e}`);
...
// v3.14.1 — arquivo .rlog do dia (AES-256-GCM, chave só em RAM)
function currentRlogFile() {
  const d = new Date();
  const y = d.getUTCFullYear();
  const m = String(d.getUTCMonth() + 1).padStart(2, "0");
  const day = String(d.getUTCDate()).padStart(2, "0");
  return path.join(LOG_DIR, `rlog-${y}-${m}-${day}.rlog`);
}

// Cada linha do .rlog é: <iv_hex>:<tag_hex>:<ct_hex>\n — inútil sem a chave.
function writeEncryptedLog(entry) {
  try {
    if (!logEncryptionKey) return; // sem chave → NÃO grava em disco (só memória via console)
    ensureLogDir();
    const file = currentRlogFile();
    // Rotação por tamanho
    try {
      const st = fs.statSync(file);
      if (st.size > LOG_FILE_MAX_BYTES) {
        fs.renameSync(file, file.replace(/\.rlog$/, `.part-${Date.now()}.rlog`));
      }
    } catch (_) {}
    const iv = crypto.randomBytes(12);
    const cipher = crypto.createCipheriv("aes-256-gcm", logEncryptionKey, iv);
    const plaintext = JSON.stringify(entry);
    const ct = Buffer.concat([cipher.update(plaintext, "utf8"), cipher.final()]);
    const tag = cipher.getAuthTag();
    const line = `${iv.toString("hex")}:${tag.toString("hex")}:${ct.toString("hex")}\n`;
    fs.appendFile(file, line, () => {});
  } catch (_) {}
}

function rotateOldRlog() {
  try {
    ensureLogDir();
    const cutoff = Date.now() - LOG_RETENTION_DAYS * 24 * 60 * 60 * 1000;
    const files = fs.readdirSync(LOG_DIR);
    for (const f of files) {
      if (!f.startsWith("rlog-") || !f.endsWith(".rlog")) continue;
...
async function loadOrCreateLogEncryptionKey() {
  if (logEncryptionKey || logKeyLoadInFlight) return;
  if (!supabase || !farmId) return;
  logKeyLoadInFlight = true;
  try {
    const newKeyHex = crypto.randomBytes(32).toString("hex");
    const { data, error } = await supabase.rpc("ensure_farm_log_key", {
      _farm_id: farmId, _new_key: newKeyHex,
    });
    if (error) { console.log("[RLOG] key rpc error", error.message); return; }
    if (typeof data === "string" && data.length >= 64) {
      logEncryptionKey = Buffer.from(data, "hex");
    }
  } catch (e) {
    console.log("[RLOG] key load exception", e && e.message);
  } finally {
    logKeyLoadInFlight = false;
  }
}

async function uploadRlogToStorage() {
  try {
    if (!supabase || !farmId) return;
    ensureLogDir();
    // Faz upload dos últimos 2 arquivos (ontem + hoje) para cobrir viradas de dia.
    const files = fs.readdirSync(LOG_DIR)
      .filter((f) => f.startsWith("rlog-") && f.endsWith(".rlog"))
      .sort()
      .slice(-2);
    for (const f of files) {
      const full = path.join(LOG_DIR, f);
      ...
      const remote = `${farmId}/${f}`;
      await supabase.storage.from("agent-logs").upload(remote, buf, {
        contentType: "application/octet-stream", upsert: true,
      });
    }
  } catch (_) { /* silent — retenta no próximo ciclo */ }
}

function startRlogUploadLoop() {
  if (rlogUploadTimer) return;
  rlogUploadTimer = setInterval(() => {
    void uploadRlogToStorage();
    rotateOldRlog();
  }, RLOG_UPLOAD_INTERVAL_MS);
  try { rlogUploadTimer.unref?.(); } catch (_) {}
}

// v3.14.1 — Mensagens exibidas na janela do agente. Whitelist genérica;
// nada de protocolo, frames, tokens, hardware, licença, etc.
function sendScreenStatus(text) {
  try {
    const entry = { ts: new Date().toISOString(), dir: "info", text: String(text) };
    liveLog.push(entry);
    if (liveLog.length > LIVE_LOG_MAX) liveLog.shift();
    if (logWindow && !logWindow.isDestroyed()) {
      try { logWindow.webContents.send("log:line", entry); } catch (_) {}
    }
  } catch (_) {}
}
...
function pushLog(level, category, message, rawFrame, humanOverride) {
  ...
  // v3.14.1 — Console (dev) + arquivo .rlog criptografado (AES-256-GCM).
  // v3.14.5 — Restaurado comportamento da 3.14.0: TX/RX/INF/ERR também vão
  // pra janela Log do tray em tempo real (além do console e do .rlog).
  if (humanMessage) {
    console.log(`[${ts}] [${category.toUpperCase()}] ${humanMessage}`);
    try {
      const dir =
        level === "error" ? "error" :
        category === "tx" || category === "raw_tx" ? "tx" :
        category === "rx" || category === "raw_rx" ? "rx" :
        "info";
      const winEntry = {
        ts: entry.timestamp,
        dir,
        text: `[${category}] ${humanMessage}${rawFrame ? " | " + rawFrame : ""}`,
      };
      liveLog.push(winEntry);
      if (liveLog.length > LIVE_LOG_MAX) liveLog.shift();
      if (logWindow && !logWindow.isDestroyed()) {
        try { logWindow.webContents.send("log:line", winEntry); } catch (_) {}
      }
    } catch (_) {}
  }
  // Log completo (com raw_frame, level, categoria) vai criptografado ao disco.
  // Sem chave em memória → nada em disco (o log fica só no console/RAM).
  writeEncryptedLog({ ...entry, message: humanMessage ?? message });
...
// Dispara o failsafe: TX com bit INVERSO ao tentado + desired_running inverso.
// Se a bomba não confirmou em SAFETY_WINDOW_MS, é sinal de modo LOCAL/botoeira
// ou hardware fora de alcance — NUNCA continuamos insistindo no comando original.
// Ações obrigatórias ao expirar:
//   1. Envia frame TX com bit inverso ao expectedBit preservando demais saídas
//   2. desired_running = bitInverso + last_actuation_origin = 'local' + safety_expired_at = now()
//   3. Limpa pending_command_id
//   4. Marca comando manual como status='timeout'
//   5. Cancela reforços pendentes
async function fireSafetyOff(equipmentId, entry) {
  const expectedBit = (entry && (entry.expectedBit === "1" || entry.expectedBit === "0")) ? entry.expectedBit : "?";
  const inverseBit = expectedBit === "1" ? "0" : "1";
  const inverseDesiredRunning = inverseBit === "1";
  const tsnn = entry && entry.tsnn ? entry.tsnn : null;
  const saida = entry && entry.saida ? entry.saida : null;
  try {
    pushLog(
      "warn",
      "system",
      `[REFORCO] Timeout ${Math.round(SAFETY_WINDOW_MS/1000)}s TSNN ${tsnn || "?"} — enviando comando inverso (safety): esperava bit=${expectedBit}, enviando bit=${inverseBit}`,
    );

    // 1) TX com bit INVERSO imediato pela serial — preserva estado das outras saídas do mesmo PLC.
    // SEM FLAGS / SEM CONDIÇÕES DE FEATURE: se há bridge e dados suficientes, envia SEMPRE.
    try {
      if (tsnn && saida && bridgeProcess && bridgeReady) {
        const inversePayload = await buildSafetyOffPayloadPreserving(tsnn, saida, inverseBit);
        const inverseFrame = `[${tsnn}_1_]{${inversePayload}}[${tsnn}_ETX_]\r`;
        sendTxFrame(inverseFrame, { priority: "reset" });
        try { rememberTxForTsnn(tsnn, "safety-inverse", (entry && entry.cmdId) || null, inverseFrame); } catch (_) {}
        pushLog("info", "tx", `[SAFETY INVERSE bit=${inverseBit}] ${inverseFrame.replace(/\r/g, "")}`, inverseFrame);
      } else {
        pushLog("warn", "system", `[SAFETY] Não foi possível enviar TX inverso (bridge indisponível ou TSNN/saida ausentes)`);
      }
    } catch (e) {
      pushLog("warn", "serial", `[SAFETY] Falha ao enviar TX inverso: ${e.message}`);
    }

    // 2) Marca o comando manual (se houver) como timeout.
    if (supabase && entry && entry.cmdId) {
      try {
        await withCloudTimeout(
          supabase.from("commands")
            .update({
              status: "timeout",
              error_message: "Bomba não confirmou em 60s — safety inverso enviado e modo LOCAL marcado",
              responded_at: new Date().toISOString(),
            })
            .eq("id", entry.cmdId)
            .in("status", ["pending", "sent"]),
          "safety timeout — mark command",
          CLOUD_WRITE_TIMEOUT_MS,
        );
      } catch (e) {
        pushLog("warn", "cloud", `[SAFETY] Falha ao marcar comando ${String(entry.cmdId).substring(0,8)} como timeout: ${e.message}`);
      }
    }

    // 3) Sempre grava desired_running com o bit inverso, marca origin=local e limpa pending_command_id.
    //    Sem exceções: se o safety expirou, o polling PRECISA parar de mandar o bit original.
    if (supabase && equipmentId) {
      try {
        const nowIso = new Date().toISOString();
        await withCloudTimeout(
          supabase.from("equipments").update({
            desired_running: inverseDesiredRunning,
            last_actuation_origin: "local",
            safety_expired_at: nowIso,
            pending_command_id: null,
            updated_at: nowIso,
          }).eq("id", equipmentId),
          `safety timeout — mark equipment local + desired=${inverseDesiredRunning}`,
          CLOUD_WRITE_TIMEOUT_MS,
        );
        pushLog(
          "warn",
          "system",
          `[SAFETY] Expirou sem confirmação — bit-inverso enviado ({${inverseBit}}), desired_running=${inverseDesiredRunning}, origin=local, pending_command_id=null`,
        );
      } catch (e) {
        pushLog("warn", "cloud", `[SAFETY] Falha ao marcar eq ${String(equipmentId).substring(0,8)} como origin=local: ${e.message}`);
      }
    }
  } catch (e) {
    ...
    safetyByEquipment.delete(equipmentId);
    clearManualReinforcements(equipmentId, "safety expirou — bit-inverso enviado");
  }
}
...
// Cache leve: por PLC hw_id → tem equipamento type='vazao' cadastrado?
// TTL 60s para pegar cadastros novos sem consultar em cada frame.
const flowEquipCache = new Map(); // plcHwId(upper) -> { hasFlow: bool, at: ms }
const FLOW_CACHE_TTL_MS = 60_000;

async function plcHasFlowEquipment(plcHwId) {
  const key = String(plcHwId || "").toUpperCase();
  if (!key) return false;
  const cached = flowEquipCache.get(key);
  if (cached && Date.now() - cached.at < FLOW_CACHE_TTL_MS) return cached.hasFlow;
  try {
    // Busca plc_groups pelo hw_id (case-insensitive)
    const { data: pg } = await withCloudTimeout(
      supabase.from("plc_groups").select("id").eq("farm_id", farmId).ilike("hw_id", key).limit(1).maybeSingle(),
      "plc lookup",
      CLOUD_TELEMETRY_TIMEOUT_MS,
    );
    let hasFlow = false;
    if (pg && pg.id) {
      const { data: eq } = await withCloudTimeout(
        supabase.from("equipments").select("id").eq("farm_id", farmId).eq("plc_group_id", pg.id).eq("type", "vazao").limit(1).maybeSingle(),
        "vazao lookup",
        CLOUD_TELEMETRY_TIMEOUT_MS,
      );
      hasFlow = !!eq;
    }
    flowEquipCache.set(key, { hasFlow, at: Date.now() });
    return hasFlow;
  } catch {
    return false;
  }
...
// Se N2 e existe equipamento type='vazao' no mesmo grupo PLC, roteia para apply_flow_telemetry.
async function processLevelReadings(rawFrame, plcHwId) {
  ...
  while ((m = RX_LEVEL_RE.exec(rawFrame)) !== null) {
    ...
    // N2 pode ser totalizador de vazão (m³) quando há equipamento type='vazao' no mesmo PLC.
    if (sensorIndex === 2 && (await plcHasFlowEquipment(plcHwId))) {
      try {
        await withCloudTimeout(
          supabase.rpc("apply_flow_telemetry", {
            _farm_id: farmId,
            _plc_hw_id: plcHwId,
            _raw_value: rawValue,
        ...
        pushLog("info", "cloud", `Vazão acumulada PLC ${plcHwId} = ${rawValue} m³`, null, null);
      } catch (e) {
        pushLog("warn", "cloud", `Vazão PLC ${plcHwId} falhou: ${e.message}`);
      }
      continue;
    }
...
function handleRxFrame(frame) {
  ...
  // v3.18.0 — confirmação da porta: só marca como "funcionando" se o frame
  // for RENOV válido (telemetria ou CFG). Ruído aleatório não conta.
  try {
    if (pendingPortConfirmation && firstRxSavedForPort !== pendingPortConfirmation) {
      const isValid = !!extractTelemetryParts(frame) || !!parseCfgResponseFrame(frame);
      if (isValid) {
        const confirmed = pendingPortConfirmation;
        firstRxSavedForPort = confirmed;
        pendingPortConfirmation = null;
        saveLastWorkingCom(confirmed);
        pushLog("info", "serial", `[COM] Porta ${confirmed} confirmada com frame válido — salva em last_working_com.txt`);
        // Atualiza agent_config se divergir da porta configurada
        try {
          if (supabase && farmId) {
            void (async () => {
              try {
                const { data } = await supabase.from("agent_config").select("serial_port").eq("farm_id", farmId).maybeSingle();
                const configured = data && data.serial_port ? String(data.serial_port).toUpperCase() : null;
                if (configured && configured !== confirmed) {
                  await supabase.from("agent_config").update({ serial_port: confirmed, updated_at: new Date().toISOString() }).eq("farm_id", farmId);
                  pushLog("info", "serial", `[COM] agent_config.serial_port atualizado de ${configured} para ${confirmed}`);
                }
              } catch (_) {}
            })();
          }
  ...
  if (cfgMatch) {
    ...
    pushLog("info", "rx", `[CFG RX] frame=${frame} tsnn=${rxTsnn} tag=${rxTag} bare=${!!cfgMatch.bare} inflight=${inflightCmd ? String(inflightCmd.id).substring(0,8) : "none"}`);
    ...
    function tryNext() {
      ...
      proc.stdout.on("data", (chunk) => {
        ...
        for (const line of lines) {
          ...
          if (!started && trimmed === "READY") {
            ...
            bridgeProcess = proc;
            bridgeReady = true;
            lastRxTimestamp = Date.now(); // grace: reinicia contagem do watchdog COM
            comAlertSent = false;
            markBridgeAlive();
            startBridgeWatchdog();
            pushLog("info", "serial", `Bridge conectada em ${portPath}`);
            if (tray) tray.setToolTip(`RENOV Agent - Online (${portPath})`);
            // v3.19.0 — sinaliza recuperação de bridge (envia recovery só se alerta prévio existia)
            void markBridgeUp();
            resolve();
            continue;
          }
      ...
      proc.on("error", (err) => {
        if (!started) {
          ...
          bridgeReady = false;
          // v3.19.0 — arma alerta de bridge caída
          markBridgeDown(err.message);
        }
      });
      ...
      proc.on("exit", (code) => {
        if (!started) {
          ...
          bridgeReady = false;
          bridgeProcess = null;
          bridgeConnected = false; // v3.20.0 — permite nova descoberta se recoverBridge falhar
          // v3.19.0 — bridge caiu inesperadamente: arma timer de alerta (60s)
          markBridgeDown(`code ${code}`);
          setTimeout(() => {
...
async function fastPathReset(cmd) {
  ...
  if (await shouldIgnoreResetDueManualReinforcement(cmd, resetTsnn)) {
    return;
  }
...
async function processNextCommand() {
  ...
  try {
    ...
    if (await shouldIgnoreResetDueManualReinforcement(cmd, expectedTsnn)) {
      processing = false;
      setTimeout(() => { void processNextCommand(); }, 50);
      return;
    }
    ...
    noteRecentManualCommand(cmd, expectedTsnn);
    if (isBackendResetCommand(cmd, frame)) markActiveReset(expectedTsnn, cmd.id);
    if (expectedTsnn) {
      recentCmdByTsnn.set(expectedTsnn, {
        cmdId: cmd.id,
...
async function downloadAndInstallAsarUpdate(version, expectedHash, expectedSize) {
  ...
  try {
    sendScreenStatus(`Atualização disponível (v${version})`);
    ...
    const stagedAsar = path.join(process.resourcesPath, "app-update.asar");
    const appFolder = path.join(process.resourcesPath, "app");
    const appFolderBak = path.join(process.resourcesPath, "app_pre_ota.bak");

    // Backup do app.asar atual (best-effort — no Windows o arquivo está
    // travado enquanto o Electron roda; o .bat vai apagar/renomear depois).
    if (originalFs.existsSync(currentAsar)) {
      pushLog("info", "update", "[OTA-asar] Backup do app.asar atual (best-effort)...");
      try {
        if (originalFs.existsSync(bakAsar)) originalFs.unlinkSync(bakAsar);
        originalFs.copyFileSync(currentAsar, bakAsar);
      } catch (e) {
        pushLog("warn", "update", `[OTA-asar] backup falhou (esperado no Windows com arquivo em uso): ${e.message}`);
      }
      ...
      try {
        if (fs.existsSync(appFolderBak)) fs.rmSync(appFolderBak, { recursive: true, force: true });
    ...
    // Move o .asar baixado para resources/app-update.asar (ao lado do app.asar).
    // No Windows não podemos sobrescrever app.asar direto — o .bat faz isso depois.
    pushLog("info", "update", `[OTA-asar] Preparando app-update.asar em ${stagedAsar}...`);
    try {
      if (originalFs.existsSync(stagedAsar)) originalFs.unlinkSync(stagedAsar);
      // tmpAsar está em userData/updates (fora do .asar) — usa fs normal para ler;
      // destino é ao lado do app.asar em resources — usa originalFs para escrever.
      originalFs.copyFileSync(tmpAsar, stagedAsar);
      try { fs.unlinkSync(tmpAsar); } catch (_) {}
    } catch (e) {
      return recordFailure(`falha ao preparar app-update.asar: ${e.message}`);
    }
    ...
    // ─────────────────────────────────────────────────────────────────
    // Windows: substituição do .asar via script .bat externo desanexado.
    // ─────────────────────────────────────────────────────────────────
    if (process.platform === "win32") {
      const batPath = path.join(process.resourcesPath, "update.bat");
      const exePath = process.execPath; // caminho absoluto do .exe do Electron
      ...
      const batContent = [
        ...
        `echo [%DATE% %TIME%] update.bat iniciado >> "%LOG%"`,
        "timeout /t 3 /nobreak >nul",
        `echo [%DATE% %TIME%] tentando remover app.asar >> "%LOG%"`,
        `del /f /q "%~dp0app.asar" >> "%LOG%" 2>&1`,
        `if exist "%~dp0app.asar" (`,
        `  echo [%DATE% %TIME%] app.asar ainda existe, aguardando 2s e tentando de novo >> "%LOG%"`,
        `  timeout /t 2 /nobreak >nul`,
        `  del /f /q "%~dp0app.asar" >> "%LOG%" 2>&1`,
        `)`,
        `if exist "%~dp0app.asar" (`,
        `  echo [%DATE% %TIME%] FALHA: nao consegui remover app.asar >> "%LOG%"`,
        `  exit /b 1`,
        `)`,
        `echo [%DATE% %TIME%] renomeando app-update.asar -> app.asar >> "%LOG%"`,
        `ren "%~dp0app-update.asar" "app.asar" >> "%LOG%" 2>&1`,
        `if not exist "%~dp0app.asar" (`,
        `  echo [%DATE% %TIME%] FALHA no rename >> "%LOG%"`,
        `  exit /b 2`,
        `)`,
        `echo [%DATE% %TIME%] reiniciando "${exeName}" >> "%LOG%"`,
        `start "" "${exeDir.replace(/\\/g, "\\\\")}\\${exeName}"`,
      ...
      try {
        originalFs.writeFileSync(batPath, batContent, { encoding: "utf8" });
      } catch (e) {
        return recordFailure(`falha ao gravar update.bat: ${e.message}`);
      }

      pushLog("info", "update", `[OTA-asar] update.bat gerado em ${batPath}, exe=${exePath}`);
      sendScreenStatus("Reiniciando para aplicar atualização...");
      pushLog("info", "update", `[OTA-asar] Atualização v${version} pronta — spawnando update.bat desanexado e encerrando`);

      // Spawn desanexado — o cmd sobrevive ao app.quit().
      try {
        ...
        return recordFailure(`falha ao spawn update.bat: ${e.message}`);
      }

      // Dá 1s para o cmd.exe iniciar antes de encerrar o Electron.
      setTimeout(() => {
        try { app.quit(); } catch (_) { try { app.exit(0); } catch (__) {} }
      }, 1000);
      return;
    }

    // Linux/macOS: fallback ao comportamento anterior (copyFileSync + relaunch).
    pushLog("info", "update", "[OTA-asar] Substituindo app.asar (non-win32)...");
    try {
      originalFs.copyFileSync(stagedAsar, currentAsar);
      try { originalFs.unlinkSync(stagedAsar); } catch (_) {}
    } catch (e) {
    ...
    sendScreenStatus("Atualização aplicada — reiniciando");
  ...
  try {
    ...
    if (error) {
      ...
      if (pollingCycleStats.startedAt > 0) {
        ...
        // v3.14.1 — mensagem genérica na tela do agente (sem tecnologia/protocolo)
        const ok = pollingCycleStats.ok;
        const fail = pollingCycleStats.fail;
        const total = ok + fail;
        if (total > 0) {
          if (fail === 0) {
            if (!lastBridgeStateWasOk) sendScreenStatus("Comunicação restabelecida");
            sendScreenStatus(`Comunicação OK (${ok}/${total})`);
            lastBridgeStateWasOk = true;
          } else if (ok === 0) {
            if (lastBridgeStateWasOk) sendScreenStatus("Alerta: comunicação interrompida — tentando recuperar");
            lastBridgeStateWasOk = false;
          } else {
            sendScreenStatus(`Comunicação OK (${ok}/${total}) — ${fail} equipamento(s) sem resposta`);
            lastBridgeStateWasOk = true;
          }
        }
      }
  ...
  try {
    ...
    commandsChannel = supabase
      ...
      .on("postgres_changes", {
        ...
        noteRecentManualCommand(cmd, extractCommandTsnn(cmd.frame || "") || cmd.plc_hw_id || null);
...
async function startAgent(cfg) {
  ...
  try {
    sendScreenStatus(`Sistema iniciado — versão ${AGENT_VERSION}`);
    ...
    // v3.19.0 — se o backend cron marcou agente offline em ciclo anterior,
    // envia recovery agora (backend também pode setar a flag via edge fn).
    try { await wdAgentRecoveredIfPending(); } catch (_) {}
    ...
    // v3.14.7 — Bridge serial NUNCA bloqueia inicialização do agente.
    // Se a COM configurada não existir, tenta auto-detectar. Se a bridge falhar
    // por qualquer motivo (Python ausente, porta ocupada, sem porta física),
    // o agente continua em modo cloud-only: heartbeat, subscriptions de
    // agent_commands e OTA continuam funcionando normalmente.
    // v3.18.0 — Descoberta de porta COM com confirmação por frame RENOV válido.
    // Ordem de tentativa:
    //   1) last_working_com.txt (última que funcionou de fato)
    //   2) agent_config.serial_port (config do banco)
    //   3) TODAS as COMs do sistema (fallback final)
    // Uma porta só é considerada "OK" se produzir extractTelemetryParts ou
    // parseCfgResponseFrame — porta aberta sem RX válido em 20s é descartada.
    try {
      startupStep = "descobrir porta COM (last_working / config / scan)";
      const CONFIRM_MS = 20_000;
      const savedPort = readLastWorkingCom();
      let sysPorts = [];
      try { sysPorts = await listSerialPortsAsync(); } catch (_) { sysPorts = []; }
      const sysPaths = sysPorts.map((p) => (p.path || "").toUpperCase()).filter(Boolean);
      const dedup = new Set();
      const candidates = [];
      const push = (p) => {
        if (!p) return;
        const up = String(p).toUpperCase();
        if (dedup.has(up)) return;
        dedup.add(up);
        candidates.push(up);
      };
      push(savedPort);
      push(comPort);
      for (const p of sysPaths) push(p);
      pushLog("info", "serial", `[COM] Descoberta — saved=${savedPort || "-"} config=${comPort || "-"} sistema=[${sysPaths.join(", ") || "vazio"}] tentativas=[${candidates.join(", ")}]`);

      // v3.20.0 — flag + handle globais para cancelar retries stale ao conectar
      bridgeConnected = false;
      if (discoveryRetryTimer) { clearTimeout(discoveryRetryTimer); discoveryRetryTimer = null; }
      let opened = false;
      for (const port of candidates) {
        if (bridgeConnected) break;
        try {
          await stopBridge();
        } catch (_) {}
        firstRxSavedForPort = null;
        pendingPortConfirmation = port;
        pushLog("info", "serial", `[COM] Testando ${port}...`);
        try {
          await startBridge(port);
        } catch (e) {
          pushLog("warn", "serial", `[COM] ${port} não abriu: ${e && e.message || e}`);
          pendingPortConfirmation = null;
          continue;
        }
        // Aguarda até CONFIRM_MS por um frame RENOV válido
        const t0 = Date.now();
        while (Date.now() - t0 < CONFIRM_MS) {
          if (firstRxSavedForPort === port) break;
          await new Promise((r) => setTimeout(r, 500));
        }
        if (firstRxSavedForPort === port) {
          comPort = port;
          opened = true;
          bridgeConnected = true;
          if (discoveryRetryTimer) { clearTimeout(discoveryRetryTimer); discoveryRetryTimer = null; }
          pushLog("info", "serial", `[COM] ${port} confirmada — usando esta porta.`);
          void wdRecoverCom(port);
          break;
        }
        pushLog("warn", "serial", `[COM] ${port} aberta mas sem frame RENOV em ${CONFIRM_MS/1000}s — descartada.`);
        pendingPortConfirmation = null;
        try { await stopBridge(); } catch (_) {}
      }

      if (!opened) {
        pushLog("warn", "system", `[CLOUD-ONLY] Nenhuma porta COM respondeu com frame RENOV. Novo scan em 30s. Agente segue em modo cloud-only (OTA/heartbeat/comandos remotos ativos).`);
        if (tray) tray.setToolTip("RENOV Agent - CLOUD-ONLY (procurando COM...)");
        // v3.19.0 — dispara alerta de COM não encontrada
        void wdAlertComNotFound();
        // Retry loop a cada 30s até achar. v3.20.0 — cancelável via bridgeConnected/discoveryRetryTimer.
        const retry = async () => {
          if (appClosing || bridgeReady || bridgeConnected) return;
          try {
            let ports2 = [];
            try { ports2 = await listSerialPortsAsync(); } catch (_) {}
            const list = ports2.map((p) => (p.path || "").toUpperCase()).filter(Boolean);
            const saved2 = readLastWorkingCom();
            const set2 = new Set();
            const cand2 = [];
            const push2 = (p) => { if (!p) return; const u = String(p).toUpperCase(); if (set2.has(u)) return; set2.add(u); cand2.push(u); };
            push2(saved2); push2(comPort); for (const p of list) push2(p);
            for (const port of cand2) {
              if (appClosing || bridgeReady || bridgeConnected) return;
              try { await stopBridge(); } catch (_) {}
              firstRxSavedForPort = null;
              pendingPortConfirmation = port;
              pushLog("info", "serial", `[COM][retry] Testando ${port}...`);
              try { await startBridge(port); } catch (e) { pendingPortConfirmation = null; continue; }
              const t0 = Date.now();
              while (Date.now() - t0 < 20_000) {
                if (firstRxSavedForPort === port) break;
                if (bridgeConnected && firstRxSavedForPort !== port) return; // outra iteração ganhou
                await new Promise((r) => setTimeout(r, 500));
              }
              if (firstRxSavedForPort === port) {
                comPort = port;
                bridgeConnected = true;
                if (discoveryRetryTimer) { clearTimeout(discoveryRetryTimer); discoveryRetryTimer = null; }
                pushLog("info", "serial", `[COM][retry] ${port} confirmada.`);
                void wdRecoverCom(port);
                return;
              }
              pendingPortConfirmation = null;
            ...
            pushLog("warn", "system", `[COM][retry] erro: ${e && e.message || e}`);
          }
          if (!bridgeConnected && !appClosing) {
            discoveryRetryTimer = setTimeout(retry, 30_000);
          }
        };
        discoveryRetryTimer = setTimeout(retry, 30_000);
      }
    } catch (bridgeErr) {
      pushLog("warn", "system", `[CLOUD-ONLY] Descoberta de porta falhou: ${bridgeErr && bridgeErr.message || bridgeErr}. Agente segue em modo cloud-only.`);
      if (tray) tray.setToolTip("RENOV Agent - CLOUD-ONLY (sem serial)");
    }
    ...
    // v3.14.1 — Carrega chave AES-256 do log em memória e inicia rotação/upload.
    startupStep = "log criptografado";
    try {
      void loadOrCreateLogEncryptionKey().then(() => {
        rotateOldRlog();
        startRlogUploadLoop();
      });
    } catch (_) {}

    // v3.14.1 — Mensagem genérica de tela: sistema pronto.
    sendScreenStatus("Sistema operando normalmente");
...
app.on("before-quit", () => {
  ...
  stopCloudReconnect();
  flushLogs();
  // Graceful shutdown: marca offline no banco imediatamente para que a web
  // atualize sem esperar o timeout de 60s do heartbeat. Best-effort: usamos
  // request síncrono curto (fetch com AbortSignal.timeout) para não travar o
  // encerramento se a internet estiver ruim.
  try {
    if (supabase && farmId) {
      const p = supabase.from("site_health").upsert(
        {
          farm_id: farmId,
          agent_status: "offline",
          com_connected: false,
          last_error: "agent_shutdown",
          last_heartbeat: new Date(Date.now() - 5 * 60 * 1000).toISOString(),
        },
        { onConflict: "farm_id" }
      );
      // Aguarda no máx 1.5s — não bloqueia o app se o backend estiver lento.
