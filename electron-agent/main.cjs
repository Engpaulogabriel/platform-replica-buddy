...
// Config viva do agente. Existe só para que o handler de set_backend/rollback e
// o contador de falhas de nuvem leiam o MESMO objeto que startAgent recebeu,
// em vez de reler o disco e divergir. Nada mais depende dela.
let runtimeCfg = null;
let pollTimer = null;
...
// Normaliza o schema de backend e reconcilia um estado de migração incompleto.
// NÃO ESCREVE NADA: instalar esta versão não pode tocar em disco, trocar URL,
// reiniciar nem testar backend. É transformação em memória, e numa instalação
// de hoje o resultado aponta para o MESMO backend de antes.
function _normalizeBackend(cfg) {
  try {
    if (!BM || !cfg) return cfg;
    const r = BM.reconcileOnBoot(cfg, new Date().toISOString());
    if (r && r.changed) {
      try { pushLog("warn", "system",
        `[BACKEND] estado de migração incompleto reconciliado: ${r.action}`); } catch (_) {}
    }
    return (r && r.config) || BM.ensureBackendConfig(cfg);
  } catch (_) { return cfg; }
}
  ...
  try {
    ...
    if (fs.existsSync(CONFIG_FILE)) {
      ...
      if (_safeAvailable()) {
        ...
        return _normalizeBackend(JSON.parse(plain));
      }
      // Ambiente sem encryption: arquivo está em texto puro
      return _normalizeBackend(JSON.parse(raw.toString("utf8")));
    }
    ...
    if (fs.existsSync(CONFIG_FILE_LEGACY)) {
      ...
      return _normalizeBackend(legacy);
    }
  } catch (e) {
...
// ═══════════════════════════════════════════════════════════════════════════
// MIGRAÇÃO DE BACKEND — wiring (PARTE 2)
// ═══════════════════════════════════════════════════════════════════════════
// ADITIVO E ISOLADO, como o Centro de Evidências. Toda a DECISÃO vive nos
// módulos puros abaixo; aqui só há I/O, despacho e restart. Nada neste bloco
// toca serial, rádio, txQueue, safety, desired_running ou qualquer constante
// operacional — trocar de nuvem não é assunto de bomba.
//
// Se um dos requires falhar, as constantes ficam null e os dois comandos novos
// respondem erro. O agente segue operando exatamente como antes.
const BM = (() => { try { return require("./lib/backendMigration.cjs"); } catch { return null; } })();
const BCHK = (() => { try { return require("./lib/backendChecks.cjs"); } catch { return null; } })();
const BPERS = (() => { try { return require("./lib/backendPersist.cjs"); } catch { return null; } })();

// Base URL/anon ativos (usa config/globais com fallback aos defaults de build).
//
// `backendConfig` entra como PRIMEIRA fonte; TODOS os fallbacks antigos
// continuam abaixo, na mesma ordem. Numa instalação de hoje `activeEndpoint()`
// devolve exatamente o que `cfg.supabaseUrl` devolvia — `ensureBackendConfig`
// promove o schema copiando o valor que já estava lá. Por isso instalar esta
// versão não muda para onde o agente fala.
function _activeBackend(cfg) {
  // Sem `backendConfig` não há nada a resolver: os fallbacks legados abaixo já
  // devolvem o mesmo valor. Sair cedo evita alocar objeto a cada chamada.
  try {
    if (!BM || !cfg || !cfg.backendConfig) return null;
    return BM.activeEndpoint(cfg);
  } catch { return null; }
}
function _activeBaseUrl(cfg) {
  const b = _activeBackend(cfg);
  return (b && b.url) || (cfg && cfg.supabaseUrl) || activeSupabaseUrl || SUPABASE_URL_DEFAULT;
}
function _activeAnon(cfg) {
  const b = _activeBackend(cfg);
  return (b && b.anonKey) || (cfg && cfg.supabaseAnonKey) || activeSupabaseAnonKey || SUPABASE_ANON_DEFAULT;
}

// ── Log seguro: NUNCA chave, token, Authorization ou payload cru ────────────
function _backendLog(nivel, etapa, extra) {
  try {
    const resumo = BM ? BM.safeSummary(runtimeCfg) : {};
    const detalhe = extra && typeof extra === "object" ? extra : {};
    pushLog(nivel, "system", `[BACKEND] ${etapa} ${JSON.stringify(
      Object.assign({}, resumo, detalhe))}`);
  } catch (_) {}
}

// Serialização idêntica à de saveConfig/writeCredentialsEnc (DPAPI quando há).
function _encodeSecureFile(obj) {
  const plain = JSON.stringify(obj, null, 2);
  return _safeAvailable() ? safeStorage.encryptString(plain) : Buffer.from(plain, "utf8");
}

// Grava config (fonte da verdade) + espelho machine-bound, atomicamente.
// Falhar aqui ABORTA a troca: o arquivo anterior fica intacto e o agente
// continua no backend antigo.
function _persistBackend(novoCfg) {
  if (!BPERS) return { ok: false, reason: "modulo_persistencia_ausente" };
  return BPERS.persistBackendConfig({
    fs, configPath: CONFIG_FILE, credsPath: CREDENTIALS_ENC_FILE,
    config: novoCfg, creds: _connectionCredsFromConfig(novoCfg),
    encode: _encodeSecureFile,
  });
}

// Evento técnico da migração. Best-effort ABSOLUTO: falha de auditoria nunca
// pode derrubar o agente nem impedir uma promoção/rollback.
function _backendTechEvent(eventType, severity, extra) {
  try {
    techEnqueueSafe(eventType, "cloud", severity,
      { payload: Object.assign({}, BM ? BM.safeSummary(runtimeCfg) : {}, extra || {}) });
  } catch (_) {}
}

// ── FALLBACK AUTOMÁTICO ────────────────────────────────────────────────────
// Só falha de NUVEM conta (cloud_auth, license_validate, api_unavailable,
// cloud_network). Rádio, PLC, serial, bridge e polling NÃO chegam aqui — o
// módulo puro recusa qualquer outro `kind`, e voltar de backend por rádio ruim
// seria trocar a causa pelo sintoma.
let backendRollbackInFlight = false;

function noteCloudOutcome(kind, ok) {
  try {
    if (!BM || !runtimeCfg || backendRollbackInFlight) return;
    const r = BM.noteOutcome(runtimeCfg, kind, ok === true);
    runtimeCfg = r.config;
    if (!r.rollback) return;
    backendRollbackInFlight = true;
    void _automaticBackendRollback(`${r.failures}/${r.limit} falhas de nuvem (${kind})`);
  } catch (_) { /* contador de nuvem nunca derruba o agente */ }
}

async function _automaticBackendRollback(motivo) {
  try {
    const r = BM.buildRollbackConfig(runtimeCfg, new Date().toISOString(), motivo);
    if (!r.ok) {
      _backendLog("error", "rollback automático IMPOSSÍVEL", { reason: r.reason });
      backendRollbackInFlight = false;
      return;
    }
    // PERSISTE ANTES DE REINICIAR. Reiniciar primeiro e gravar depois deixaria
    // o agente subindo no backend quebrado — exatamente o que o rollback existe
    // para evitar.
    const p = _persistBackend(r.config);
    if (!p.ok) {
      _backendLog("error", "rollback automático NÃO persistiu — segue no backend atual",
        { reason: p.reason });
      backendRollbackInFlight = false;
      return;
    }
    runtimeCfg = r.config;
    _backendLog("warn", "rollback automático aplicado", { motivo });
    _backendTechEvent("backend_rollback_auto", "critical", { motivo });
    await relaunchAgent("backend_rollback", 0);
  } catch (e) {
    _backendLog("error", "rollback automático falhou", { erro: formatError(e) });
    backendRollbackInFlight = false;
  }
}

// ── set_backend ────────────────────────────────────────────────────────────
async function handleSetBackend(cmd, startedAt) {
  if (!BM || !BCHK || !BPERS) {
    await resolveAgentCommand(cmd.id, "error", { error: "Módulos de migração indisponíveis nesta versão" });
    return;
  }
  // 15) TTL — o caminho Realtime entrega o INSERT sem passar pelo filtro de
  // `expires_at` das queries. Para um comando que troca o backend da fazenda,
  // a checagem tem de ser explícita.
  if (BM.isAgentCommandExpired(cmd, Date.now())) {
    _backendLog("warn", "set_backend RECUSADO — comando expirado", { cmd: cmd.id.substring(0, 8) });
    await resolveAgentCommand(cmd.id, "error", { error: "Comando expirado — não executado" });
    return;
  }

  const v = BM.validateSetBackendPayload(cmd.payload || {});
  if (!v.ok) {
    // Payload inválido NÃO toca config, NÃO chama rede.
    _backendLog("warn", "set_backend RECUSADO — payload inválido", { errors: v.errors });
    await resolveAgentCommand(cmd.id, "error", {
      duration_ms: Date.now() - startedAt,
      error: `Payload inválido: ${v.errors.join(", ")}`,
    });
    return;
  }
  const alvo = v.value;
  const cfg = runtimeCfg || loadConfig();
  if (!cfg) {
    await resolveAgentCommand(cmd.id, "error", { error: "Config local indisponível" });
    return;
  }
  runtimeCfg = BM.ensureBackendConfig(cfg);

  // Idempotência ANTES da rede: reprocessar o mesmo migration_id não pode nem
  // sequer bater no candidato de novo.
  const previa = BM.decidePromotion(
    Object.fromEntries(BM.CHECKS.map((c) => [c, true])), runtimeCfg, alvo.migrationId);
  if (previa.idempotent) {
    _backendLog("info", "set_backend no-op", { reason: previa.reason });
    await resolveAgentCommand(cmd.id, "done", {
      duration_ms: Date.now() - startedAt,
      data: { skipped: previa.reason, migration_id: alvo.migrationId },
    });
    return;
  }

  // 6/7) CHECAGENS REAIS contra o CANDIDATO, sem tocar em config, sessão ou
  // token. O agente segue falando com o backend antigo esta linha inteira.
  _backendLog("info", "validando candidato", {
    candidateHost: (() => { try { return new URL(alvo.primaryUrl).hostname; } catch { return null; } })(),
    migration_id: alvo.migrationId,
  });
  let fp = {};
  try { fp = getCachedFingerprint(); } catch (_) { fp = {}; }
  const rel = await BCHK.runBackendChecks(
    { url: alvo.primaryUrl, anonKey: alvo.primaryAnonKey },
    { email: runtimeCfg.email, password: runtimeCfg.password,
      licenseToken: runtimeCfg.licenseToken || null,
      machineIdHash: fp.machine_id_hash, fingerprint: fp.fingerprint,
      agentVersion: AGENT_VERSION },
    { fetch, timeoutMs: alvo.validationTimeoutMs });

  const d = BM.decidePromotion(rel.results, runtimeCfg, alvo.migrationId);
  if (!d.promote) {
    _backendLog("warn", "promoção RECUSADA", { reason: d.reason, failed: d.failed || null, details: rel.details });
    await resolveAgentCommand(cmd.id, "error", {
      duration_ms: Date.now() - startedAt,
      error: `Validação falhou: ${(d.failed || [d.reason]).join(", ")}`,
      data: { results: rel.results, details: rel.details },
    });
    return;
  }

  const built = BM.buildPromotedConfig(runtimeCfg, alvo, new Date().toISOString());
  if (!built.ok) {
    _backendLog("error", "promoção abortada na montagem do config", { reason: built.reason });
    await resolveAgentCommand(cmd.id, "error", { error: `Config não montado: ${built.reason}` });
    return;
  }

  // 9) PERSISTÊNCIA ATÔMICA — e só depois dela existe promoção.
  const p = _persistBackend(built.config);
  if (!p.ok) {
    _backendLog("error", "persistência FALHOU — backend NÃO trocado, sem restart", { reason: p.reason });
    _backendTechEvent("backend_promotion_failed", "error", { etapa: "persistencia", reason: p.reason });
    await resolveAgentCommand(cmd.id, "error", {
      duration_ms: Date.now() - startedAt,
      error: `Persistência falhou (${p.reason}) — operação mantida no backend atual`,
    });
    return;
  }

  runtimeCfg = built.config;
  _backendLog("warn", "backend PROMOVIDO — reiniciando", { migration_id: alvo.migrationId });
  _backendTechEvent("backend_promoted", "critical", { migration_id: alvo.migrationId });
  // Resolve no backend ANTIGO (foi de lá que o comando veio) antes do restart.
  await resolveAgentCommand(cmd.id, "done", {
    duration_ms: Date.now() - startedAt,
    data: { promoted: true, migration_id: alvo.migrationId,
            summary: BM.safeSummary(built.config), mirror: !!(p.mirror && p.mirror.ok) },
  });
  setTimeout(() => { void relaunchAgent("set_backend", 0); }, 800);
}

// ── rollback_backend ───────────────────────────────────────────────────────
async function handleRollbackBackend(cmd, startedAt) {
  if (!BM || !BPERS) {
    await resolveAgentCommand(cmd.id, "error", { error: "Módulos de migração indisponíveis nesta versão" });
    return;
  }
  if (BM.isAgentCommandExpired(cmd, Date.now())) {
    _backendLog("warn", "rollback_backend RECUSADO — comando expirado", { cmd: cmd.id.substring(0, 8) });
    await resolveAgentCommand(cmd.id, "error", { error: "Comando expirado — não executado" });
    return;
  }
  const cfg = runtimeCfg || loadConfig();
  ...
  // Idempotente: já está no fallback → nada a fazer, e responde sucesso.
  if (runtimeCfg.backendConfig.activeBackend === "fallback") {
    _backendLog("info", "rollback_backend no-op (já no fallback)");
    await resolveAgentCommand(cmd.id, "done", {
      duration_ms: Date.now() - startedAt,
      data: { skipped: "ja_no_fallback", summary: BM.safeSummary(runtimeCfg) },
    });
    return;
  }

  const motivo = String((cmd.payload && cmd.payload.reason) || "rollback manual").slice(0, 120);
  const r = BM.buildRollbackConfig(runtimeCfg, new Date().toISOString(), motivo);
  if (!r.ok) {
    _backendLog("error", "rollback RECUSADO", { reason: r.reason });
    await resolveAgentCommand(cmd.id, "error", {
      duration_ms: Date.now() - startedAt,
      error: `Rollback recusado: ${r.reason}`,
    });
    return;
  }
  const p = _persistBackend(r.config);
  if (!p.ok) {
    _backendLog("error", "rollback NÃO persistiu — sem restart", { reason: p.reason });
    await resolveAgentCommand(cmd.id, "error", {
      duration_ms: Date.now() - startedAt,
      error: `Persistência falhou (${p.reason}) — operação mantida no backend atual`,
    });
  ...
  runtimeCfg = r.config;
  _backendLog("warn", "rollback manual aplicado — reiniciando", { motivo });
  _backendTechEvent("backend_rollback_manual", "critical", { motivo });
  await resolveAgentCommand(cmd.id, "done", {
    duration_ms: Date.now() - startedAt,
    data: { rolled_back: true, summary: BM.safeSummary(r.config) },
  });
  setTimeout(() => { void relaunchAgent("rollback_backend", 0); }, 800);
}
...
async function connectCloudServices(cfg, options = {}) {
  ...
  try {
    ...
    // Nuvem respondeu e autenticou: zera o contador de falhas do backend.
    noteCloudOutcome("cloud_auth", true);
    return true;
  } catch (e) {
    supabase = null;
    if (!quiet) pushLog("warn", "cloud", `Nuvem indisponível; COM continua local: ${formatError(e)}`);
    // Falha de NUVEM (login/rede) — a única classe que pode disparar rollback.
    // A operação serial segue intocada; o COM continua local, como sempre.
    noteCloudOutcome("cloud_network", false);
    startCloudReconnect(cfg);
    return false;
  }
}
...
async function handleAgentCommand(cmd) {
  ...
  try {
    switch (cmd.kind) {
      ...
      // MIGRAÇÃO DE BACKEND — comandos SEPARADOS do OTA. `update_agent` nunca
      // troca backend, e `set_backend` nunca atualiza versão.
      case "set_backend": {
        await handleSetBackend(cmd, startedAt);
        break;
      }
      case "rollback_backend": {
        await handleRollbackBackend(cmd, startedAt);
        break;
      }
...
async function tickAgentCommandPoll() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .order("created_at", { ascending: true })
      .limit(10);
    // Este poll é o sinal canônico de "a API REST responde". Erro aqui é
    // api_unavailable; resposta (mesmo vazia) zera o contador.
    noteCloudOutcome("api_unavailable", !error);
    if (error) return;
...
async function startAgent(cfg) {
  ...
  // Config viva para o wiring de backend (contador de falhas de nuvem e os
  // comandos set_backend/rollback_backend). Não altera nada do fluxo abaixo.
  try { runtimeCfg = BM ? BM.ensureBackendConfig(cfg) : cfg; } catch (_) { runtimeCfg = cfg; }
...
async function validateLicenseHeartbeat(cfg) {
  ...
  try {
    ...
    if (resp.ok && body?.valid) {
      ...
      _saveLicenseGrace();
      noteCloudOutcome("license_validate", true);
      return;
    }
    ...
    // Rede/timeout ao validar licença — falha de NUVEM, conta para o rollback.
    // O caminho 403/revoked acima NÃO passa por aqui e segue intocado.
    noteCloudOutcome("license_validate", false);
  }
