...
# --- 1c) sincroniza lib/ -> app/lib/ (raiz do asar) -----------------------
# DEFEITO CORRIGIDO AQUI: `asar pack app` empacota SOMENTE o conteudo de app/.
# Os modulos de lib/ (technicalEvents.cjs do Centro de Evidencias e os tres da
# migracao de backend) nunca eram copiados, entao o asar de distribuicao saia
# SEM eles. Em runtime os require() caem no catch e viram null — sem erro
# visivel, so a funcionalidade morta. Verificado empiricamente: o asar gerado
# antes desta correcao nao continha /lib em lugar nenhum.
LIB_SRC="$AGENT_DIR/lib"
if [ -d "$LIB_SRC" ]; then
  rm -rf "$APP_DIR/lib"
  mkdir -p "$APP_DIR/lib"
  cp "$LIB_SRC"/*.cjs "$APP_DIR/lib/" 2>/dev/null || true
  for f in "$APP_DIR/lib"/*.cjs; do node --check "$f"; done
  echo "[build-secure] lib/ sincronizada -> app/lib/ ($(ls -1 "$APP_DIR/lib" | wc -l | tr -d ' ') modulo(s))"
else
  echo "[build-secure] AVISO: $LIB_SRC nao existe — nenhum modulo auxiliar empacotado"
fi
...
# --- 6a2) VERIFICACAO OBRIGATORIA: os modulos de lib/ estao DENTRO do asar --
# Sem esta trava o build volta a sair silenciosamente sem lib/ — que foi
# exatamente como o defeito passou despercebido ate agora.
if [ -d "$LIB_SRC" ]; then
  MISSING=""
  for f in "$LIB_SRC"/*.cjs; do
    b="$(basename "$f")"
    (cd "$AGENT_DIR" && $ASAR list "$OUT" 2>/dev/null) | grep -qx "/lib/$b" || MISSING="$MISSING $b"
  done
  if [ -n "$MISSING" ]; then
    echo "[build-secure] ERRO CRITICO: modulos ausentes no asar:$MISSING"
    echo "               Build ABORTADO (require() cairia em null silenciosamente)."
    rm -f "$OUT"
    exit 7
  fi
  echo "[build-secure] modulos de lib/ confirmados dentro do asar"
fi
