...
export default function InemaReportTab({ farmId, fromDate, toDate }: Props) {
  ...
  const handleXLSX = async () => {
    const g = await guardExport("inema", "xlsx", rowsWithAcc.length);
    if (!g.allowed) { notify.fail("INEMA", "Limite de exportações atingido. Tente novamente mais tarde."); return; }
  ...
  const handleNormalPDF = async () => {
    const g = await guardExport("inema", "pdf", rowsWithAcc.length);
    if (!g.allowed) { notify.fail("INEMA", "Limite de exportações atingido. Tente novamente mais tarde."); return; }
