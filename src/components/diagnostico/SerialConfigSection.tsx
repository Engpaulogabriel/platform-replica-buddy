...
const SerialConfigSection = () => {
  ...
  const refreshPorts = async () => {
    try {
      addDiag("serialAPI existe? " + !!(window as any).serialAPI);
      if (!(window as any).serialAPI) {
        addDiag("ERRO: serialAPI não disponível");
        notify.fail("Porta Serial", "serialAPI não disponível (execute no Electron)");
        return;
      }
      addDiag("Chamando serialAPI.list()...");
      const list = await (window as any).serialAPI.list();
      addDiag("Portas encontradas: " + JSON.stringify(list.map((p: any) => p.path)));
      setPorts(list.map((p: any) => ({ path: p.path, manufacturer: p.manufacturer })));
      if (list.length === 0) addDiag("Lista vazia — módulo nativo pode não ter carregado");
      notify.tip("Porta Serial", "Portas atualizadas");
    } catch (e: any) {
      addDiag("ERRO em list(): " + (e?.message || String(e)));
  ...
  useEffect(() => {
    ...
    addDiag("window.serialAPI: " + (typeof (window as any).serialAPI));
    refreshPorts();
    setConnected(!!(window as any).serialAPI?.isOpen?.());

    const unsub = (window as any).serialAPI?.onStatus?.((evt: any) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      await (window as any).serialAPI!.open({ path: port, baudRate, dataBits, parity, stopBits });
  ...
  const handleDisconnect = async () => {
    try {
      await (window as any).serialAPI!.close();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-5">
          <h3 className="text-sm font-semibold text-foreground">Parâmetros Avançados</h3>
          <div className="space-y-2">
            <Label className="text-sm">Data Bits</Label>
            <Select value={dataBits} onValueChange={setDataBits}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {["7", "8"].map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
      ...
      {/* Log Serial */}
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="text-sm font-semibold text-foreground mb-3">Log Serial</h3>
