...
export function SwitchingProtectionPanel() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) notify.fail("Proteções", "Não foi possível carregar as proteções.");
    setRows((data as Row[]) ?? []);
    setLoading(false);
  }, [farmId]);
  ...
  const toggle = async (row: Row, next: boolean) => {
    ...
    setSaving(null);
    if (error) { notify.fail("Proteções", "Não foi possível alterar a proteção deste poço."); return; }
    notify.ok("Proteções", next
  ...
  const setSeconds = async (row: Row, secs: number) => {
    ...
    setSaving(null);
    if (error) { notify.fail("Proteções", "Não foi possível alterar a janela."); return; }
    void load();
  };
