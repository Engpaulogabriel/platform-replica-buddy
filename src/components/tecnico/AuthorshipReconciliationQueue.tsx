...
import { RefreshCw, UserCheck, Lock, AlertTriangle, ShieldCheck, HelpCircle } from "lucide-react";
import { useDefaultFarmId } from "@/hooks/useDefaultFarmId";
import { useFarmAccess } from "@/hooks/useFarmAccess";
import { usePlatformAdmin } from "@/hooks/usePlatformAdmin";
import { toast } from "sonner";
import type { EvidenceStrength } from "@/lib/authorship/reconciliation";
...
interface QueueRow {
  ...
  suggestion_strength: EvidenceStrength | null; suggestion_source: string | null;
  status: string;
}
...
// Fonte legível para a coluna de evidência.
const sourceLabel = (src: string | null) => {
  if (!src) return "sem fonte registrada";
  if (src.startsWith("nome_ja_presente_no_lote:")) {
    const origem = src.split(":")[1] || "legado_sem_fonte";
    return `nome já presente no lote (origem: ${origem === "legado_sem_fonte" ? "legado, sem fonte registrada" : origem})`;
  }
  return src;
};
...
export default function AuthorshipReconciliationQueue() {
  ...
  // Lotes elegíveis à confirmação em massa: sugestão apenas corroborada pelo lote.
  const corroborated = useMemo(
    () => rows.filter((r) => r.suggested_user && (r.suggestion_strength ?? "corroborated_batch") === "corroborated_batch"),
    [rows],
  );
  const selectedIds = useMemo(() => Object.keys(selected).filter((k) => selected[k]), [selected]);

  async function applyRow(row: QueueRow, userId: string, confirmCorroborated: boolean) {
    const { data: auth } = await supabase.auth.getUser();
    const executor = auth?.user?.id;
    if (!executor) throw new Error("Sessão sem usuário — refaça o login.");
    const strength = row.suggestion_strength ?? "corroborated_batch";
    const { data, error } = await supabase.rpc("apply_remote_reconciliation" as any, {
      ...
      _executor: executor,
      _evidence: `Reconciliação em lote pelo Setor Técnico — ${row.events_unnamed} evento(s) do lote ${row.batch_id} · evidência ${strength} (${sourceLabel(row.suggestion_source)})`,
      _confirm_corroborated: confirmCorroborated,
    });
  ...
  async function apply(row: QueueRow) {
    ...
    const isCorroborated =
      userId === row.suggested_user && (row.suggestion_strength ?? "corroborated_batch") === "corroborated_batch";
    if (isCorroborated && !isPlatformAdmin) {
      toast.error("Sugestão apenas corroborada pelo lote — confirmação restrita a platform_admin.");
      return;
    }
    setBusy(row.id);
    try {
      const n = await applyRow(row, userId, isCorroborated);
      toast.success(`${n} evento(s) atribuídos a ${nameOf(userId)}.`);
      await load();
    } catch (e) {
      toast.error(`Não aplicado: ${(e as Error).message}`);
    } finally { setBusy(null); }
  }

  async function applyBulk() {
    const targets = corroborated.filter((r) => selected[r.id]);
    if (!targets.length) return;
    setBulkBusy(true);
    let ok = 0, fail = 0;
    for (const r of targets) {
      try { await applyRow(r, choice[r.id] ?? r.suggested_user!, true); ok++; }
      catch { fail++; }
    }
    toast[fail ? "warning" : "success"](`${ok} lote(s) confirmados${fail ? `, ${fail} falharam` : ""}.`);
    setBulkBusy(false);
    await load();
  }
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              ...
              <br />
              <strong>Forte</strong> = trilha direta (auditoria de comando, comando, dados do evento, WhatsApp
              ou reconciliação já aprovada). <strong>Corroborada pelo lote</strong> = o nome veio apenas de
              outro evento do mesmo lote (fonte indireta/legada) e exige confirmação humana.
            </p>
          </div>
          <div className="flex items-center gap-2">
            {isPlatformAdmin && corroborated.length > 0 && (
              <Button size="sm" variant="secondary" disabled={bulkBusy || selectedIds.length === 0}
                      onClick={() => void applyBulk()}>
                {bulkBusy ? "Confirmando…" : `Confirmar ${selectedIds.length} lote(s) corroborados`}
              </Button>
            )}
            ...
            <Table className="text-xs">
              <TableHeader><TableRow>
                <TableHead className="w-8">
                  {isPlatformAdmin && corroborated.length > 0 && (
                    <Checkbox
                      checked={selectedIds.length > 0 && selectedIds.length === corroborated.length}
                      onCheckedChange={(v) =>
                        setSelected(v ? Object.fromEntries(corroborated.map((r) => [r.id, true])) : {})}
                      aria-label="Selecionar todos os lotes corroborados"
                    />
                  )}
                </TableHead>
                <TableHead>Período (BRT)</TableHead><TableHead>Ação</TableHead>
                <TableHead>Eventos</TableHead><TableHead>Sem nome</TableHead>
                <TableHead>Evidência / fonte</TableHead>
                <TableHead>Candidatas</TableHead><TableHead>Responsável</TableHead><TableHead />
              </TableRow></TableHeader>
              <TableBody>
                {rows.map((r) => {
                  const conflito = (r.candidates?.length ?? 0) > 1;
                  const strength = r.suggested_user ? (r.suggestion_strength ?? "corroborated_batch") : null;
                  const isCorr = strength === "corroborated_batch";
                  return (
                    <TableRow key={r.id}>
                      <TableCell>
                        {isPlatformAdmin && isCorr && (
                          <Checkbox
                            checked={!!selected[r.id]}
                            onCheckedChange={(v) => setSelected((s) => ({ ...s, [r.id]: !!v }))}
                            aria-label={`Selecionar lote ${r.batch_id}`}
                          />
                        )}
                      </TableCell>
                      ...
                      <TableCell className="min-w-[220px]">
                        {strength === "strong" && (
                          <Badge variant="outline" className="bg-primary/10 text-primary border-primary/40">
                            <ShieldCheck className="w-3 h-3 mr-1" /> Forte
                          </Badge>
                        )}
                        {isCorr && (
                          <Badge variant="outline" className="bg-warning/10 text-warning border-warning/40">
                            <HelpCircle className="w-3 h-3 mr-1" /> Corroborada pelo lote
                          </Badge>
                        )}
                        {!strength && <span className="text-muted-foreground">sem sugestão</span>}
                        <div className="text-[10px] text-muted-foreground mt-0.5">
                          Fonte: {sourceLabel(r.suggestion_source)}
                        </div>
                      </TableCell>
                      <TableCell>
                        {conflito && (
                      ...
                      <TableCell>
                        <Button size="sm" disabled={busy === r.id} onClick={() => void apply(r)}>
                          {busy === r.id ? "Aplicando…" : isCorr ? "Confirmar lote" : "Aplicar ao lote"}
                        </Button>
                      </TableCell>
                    </TableRow>
                  );
