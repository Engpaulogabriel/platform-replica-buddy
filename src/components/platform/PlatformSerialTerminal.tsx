...
export default function PlatformSerialTerminal({ isAdmin }: { isAdmin: boolean }) {
  ...
  const loadFarms = async () => {
    ...
    if (list.length === 0) {
      ...
      if (ferr) { push("err", `Nao consegui carregar fazendas: ${ferr.message}`); return; }
  ...
  const loadEquips = async (id: string) => {
    ...
    const { data, error } = await supabase
      ...
      .eq("active", true)
      .order("name");
    if (error) { push("err", error.message); return; }
    ...
    try {
      ...
      const { data: ins, error: insErr } = await supabase
        ...
        .insert({
          farm_id: farmId,
          kind: "serial_sniff",
          payload: { duration_ms: durationMs } as never,
          created_by: userRes?.user?.id ?? null,
      ...
      while (Date.now() < deadline) {
        ...
        const { data: row } = await supabase
          ...
          .maybeSingle();
        if (!row) continue;
        const d = (row.result as { data?: { frames?: { at: string; frame: string }[] } } | null)?.data;
        if (d?.frames) {
        ...
        if (row.status === "done" || row.status === "error" || row.status === "expired") break;
      }
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            <Badge variant="outline" className="ml-1 font-normal">substitui o Hercules</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {mode === "raw" && (
            <div className="space-y-2">
              <Label className="text-xs">Frame / comando (o agente adiciona CR automaticamente)</Label>
            ...
            <div className="space-y-3">
                  ...
                  <SelectContent>
                    {equips.map((e) => (
                      <SelectItem key={e.id} value={e.id}>
                        {e.name} - PLC {(e.hw_id || "").slice(0, 4)} / saida {e.saida ?? "-"}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {selectedEquip && (
                <div className="text-xs text-muted-foreground font-mono">
                  Endereco PLC: {(selectedEquip.hw_id || "").slice(0, 4)} - Saida: {String(selectedEquip.saida ?? "-").padStart(2, "0")} - Tipo: {selectedEquip.type}
                </div>
              )}
              ...
              <p className="text-xs text-amber-600">
                Ligar/Desligar aqui e teste de diagnostico (igual ao Hercules): nao grava o estado no sistema, entao o polling pode reverter em ~30s.
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 border-t pt-3">
            ...
            <span className="text-sm text-muted-foreground">Ver trafego passando (passivo):</span>
            <Button size="sm" variant="secondary" disabled={sniffing || !farmId} onClick={() => startSniff(10_000)}>
              {sniffing ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : null} Captura 10s
            </Button>
      ...
      <Card>
        <CardHeader className="pb-2 flex-row items-center justify-between">
          <CardTitle className="text-sm">Log TX / RX ({log.length}/{MAX_LOG})</CardTitle>
        ...
        <CardContent>
          <div ref={logRef} className="h-72 overflow-y-auto rounded bg-black/90 p-3 font-mono text-xs leading-relaxed">
            {log.length === 0 && <div className="text-muted-foreground">Sem trafego ainda.</div>}
            {log.map((l, i) => (
              <div key={i} className={
      ...
      <AlertDialog open={!!confirm} onOpenChange={(o) => { if (!o) setConfirm(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar comando sensivel</AlertDialogTitle>
            <AlertDialogDescription>
              Voce esta prestes a enviar <code className="font-mono">{confirm?.frame}</code> pela serial
