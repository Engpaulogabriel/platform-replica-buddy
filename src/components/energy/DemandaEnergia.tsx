...
export default function DemandaEnergia() {
  ...
  const handleSaveDemand = async () => {
    ...
    try {
      const routed = tryGetSupabaseForFarm(farmId);
      if (!routed.client) throw new Error("Servidor desta fazenda indisponível.");
      const { error } = await routed.client
        .from("farm_productivity_config" as any)
  ...
  const handleShedLoad = async () => {
    ...
    try {
      // COMANDO FÍSICO. Escrita operacional: fail-closed pelo farm_id — jamais
      // enfileirar no backend antigo o desligamento de uma bomba cujo Agent
      // escuta o novo (o comando simplesmente nunca seria executado).
