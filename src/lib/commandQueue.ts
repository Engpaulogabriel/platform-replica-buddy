...
// DUAL-BACKEND: fluxos operacionais resolvem o cliente UMA VEZ, a partir do
// farmId da própria operação. O singleton acima segue sendo o backend ANTIGO.
// A SEMÂNTICA DO COMANDO NÃO MUDA: continua INSERT direto em public.commands,
// exatamente como hoje. Muda apenas o DESTINO.
...
async function resolveCommandUserId(provided?: string | null, client?: RenovSupabase): Promise<string> {
  ...
  // created_by TEM de ser o auth.uid() do backend que recebe o INSERT (RLS).
