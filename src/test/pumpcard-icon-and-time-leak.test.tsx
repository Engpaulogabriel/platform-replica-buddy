...
async function draw(p: Pump, extra: Record<string, unknown> = {}) {
  ...
  const r = render(
    <TechnicalTelemetryProvider><PumpCard {...(props(p, extra) as unknown as React.ComponentProps<typeof PumpCard>)} /></TechnicalTelemetryProvider>);
...
describe("ícone de atualização original", () => {
  ...
  it("3. após confirmação: CheckCircle2, e depois volta ao RefreshCw", async () => {
    ...
    rerender(<TechnicalTelemetryProvider><PumpCard {...(props(base()) as unknown as React.ComponentProps<typeof PumpCard>)} /></TechnicalTelemetryProvider>);
    expect(icone()).toContain("lucide-refresh-cw");
  });
...
describe("6. chave DESLIGADA: zero tempo técnico no card", () => {
  ...
  it("o popover mostra o HISTÓRICO, mas não a idade da telemetria", async () => {
    // Data/hora de comandos e leituras é histórico operacional e vale para
    // todo perfil. O que a chave governa é só a IDADE da comunicação.
    db.admin = true;
    await draw(base());
    fireEvent.click(botao());
    ...
    expect(surface()).not.toMatch(/sem comunica[çc][ãa]o h[áa]/i);
  });
