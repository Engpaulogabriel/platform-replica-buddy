...
export const Cadastros = () => {
  ...
  const openEditEquip = (e: CloudEquipamento) => {
    ...
    setEquipForm({
      ...
      is_captacao: (e as unknown as { is_captacao?: boolean }).is_captacao ?? (e.type === "poco"),
  ...
  const saveEquip = async () => {
    ...
    const input = {
      ...
      is_captacao: equipForm.is_captacao,
            ...
            {equipForm.type !== "nivel" && (
              <div className="flex items-start gap-3 rounded-md border border-border bg-secondary/40 p-3">
                <input
                  id="is_captacao"
                  type="checkbox"
                  checked={equipForm.is_captacao}
                  onChange={(e) => setEquipForm({ ...equipForm, is_captacao: e.target.checked })}
                  className="mt-1 h-4 w-4 rounded border-border accent-primary cursor-pointer"
                />
                <div className="flex-1">
                  <Label htmlFor="is_captacao" className="text-sm text-foreground cursor-pointer">
                    Equipamento de Captação (INEMA)
                  </Label>
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Marque para bombas/conjuntos que <strong>captam água</strong> (poços, bombas de rio, conjuntos de captação).
                    Boosters e recalques de distribuição devem ficar <strong>desmarcados</strong>.
                    Apenas os marcados entram no Relatório INEMA.
                  </p>
                </div>
              </div>
            )}
