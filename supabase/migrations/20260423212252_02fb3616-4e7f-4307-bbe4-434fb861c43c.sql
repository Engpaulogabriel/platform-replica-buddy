...
BEGIN
  ...
  -- Verifica se o command_id passado é do tipo MANUAL
  -- (polling não conta como atuação remota — só reforça/lê estado)
  ...
  IF v_state_changed THEN
    IF v_pending_confirms_expected THEN
      -- Resposta de comando manual pendente confirma a mudança esperada
      v_origin := 'remote';
    ELSIF v_command_is_manual THEN
      -- _command_id passado é de comando manual
      v_origin := 'remote';
    ELSE
      -- Sem confirmação por pending nem por command_id manual:
      -- verifica se houve comando manual ENVIADO nos últimos 60s.
      -- (polling NÃO conta — só comando manual real conta como atuação remota)
      ...
      IF v_recent_remote_command_at IS NOT NULL THEN
        v_origin := 'remote';
      ELSE
        -- Mudou sem nenhum comando manual recente → atuação local
        v_origin := 'local';
