...
BEGIN
  ...
  recent AS (
    ...
    FROM sensors
    WHERE percent_now IS NOT NULL
  ),
  -- Pega a leitura histórica mais ANTIGA disponível (até 24h atrás),
  -- exigindo no mínimo 5 minutos de janela para taxa estável.
  anchor AS (
  ...
  -- Fallback: se não houve âncora histórica, ao menos mostra o nível atual
  ...
  RETURN jsonb_build_object(
    ...
    'window_minutes', CASE WHEN v_window_minutes IS NULL THEN NULL ELSE round(v_window_minutes, 0) END,
    'calculated_at', now()
  );
END;
$$;