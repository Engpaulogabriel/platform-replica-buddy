...
BEGIN
  ...
  LOOP
    ...
    -- ─────────────────────────────────────────────────────────────────────
    -- CLASSIFICAÇÃO DE ORIGEM (atualizada):
    -- O alerta de "Acionamento Local" deve ser STICKY: só é limpo quando
    -- houver um comando remoto MANUAL enviado pela plataforma E confirmado
    -- pela resposta da PLC (v_pending_confirms_expected, sem ser reset
    -- protetivo). Em qualquer outra situação (polling normal, desired_running
    -- batendo por sincronização pós-local, etc.) o 'local' é preservado.
    -- ─────────────────────────────────────────────────────────────────────
    IF v_pending_confirms_expected AND NOT v_pending_is_protective_reset THEN
      -- Confirmação real de comando manual remoto: limpa local.
      v_origin := 'remote';
    ...
    ELSIF COALESCE(v_eq.last_actuation_origin, '') = 'local' THEN
      -- Já está local e nenhum comando remoto manual foi confirmado:
      -- preserva 'local' independentemente do que o agente reportou.
      v_origin := NULL;
