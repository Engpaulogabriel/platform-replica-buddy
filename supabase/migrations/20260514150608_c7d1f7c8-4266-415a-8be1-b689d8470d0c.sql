...
BEGIN
  ...
  metrics AS (
    SELECT
      ...
      COALESCE((
        SELECT COUNT(*)::int FROM post_per_pump
        WHERE first_on <= cycle_start + interval '5 minutes'
      ), 0) AS post_ok_v
    FROM operated o
    LEFT JOIN post_per_pump pp ON pp.equipment_id = o.equipment_id
  )
  SELECT
    _date AS cycle_date,
    CASE
      ...
      ELSE ROUND(GREATEST(0::numeric, LEAST(100::numeric,
        100::numeric - (
          ...
          * 100::numeric
          / GREATEST(1::numeric, COALESCE(m.pumps_operated_v, 0)::numeric * 1260::numeric)
        )
      )), 1)
    ...
    (COALESCE(m.post_lost_pump_min_v, 0)
