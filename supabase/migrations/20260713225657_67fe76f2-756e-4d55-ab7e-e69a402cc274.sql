...
-- BUG 1: Trigger de alerta infere origem real (comando recente = remoto, senão local)
...
-- BUG 2: Função que detecta comandos não obedecidos e alerta
...
BEGIN
  ...
  LOOP
    ...
    UPDATE commands
       SET status = 'timeout',
           responded_at = NOW(),
           error_message = COALESCE(error_message, 'Não obedeceu ao comando — possivelmente em modo Local')
     WHERE id = _cmd.id;
  END LOOP;
END;
$$;

-- Agendar via pg_cron (a cada 1 minuto — menor granularidade suportada)
DO $$
BEGIN
...
-- BUG 3: Guarda no INSERT de commands — cancela comandos de atuação redundantes
-- quando equipamento está em modo LOCAL e o estado já bate com o desejado.
...
DECLARE
  _eq RECORD;
  _has_actuation boolean;
  _payload_bit char(1);
  _saida_idx int;
  _current_bit char(1);
BEGIN
  -- Apenas frames de atuação [TSNN_1_]{...}
  IF NEW.frame IS NULL OR NEW.frame !~ '\[\d{4}_1_\]\{[01]{1,6}\}' THEN
    RETURN NEW;
  END IF;
  IF NEW.type NOT IN ('automation','manual','polling') THEN
    RETURN NEW;
  END IF;
  ...
  SELECT id, saida, last_actuation_origin, last_confirmed_state, last_outputs_state
    INTO _eq
    FROM equipments WHERE id = NEW.equipment_id;
  IF NOT FOUND THEN RETURN NEW; END IF;

  -- Só bloqueia se equipamento está confirmadamente em Local
  IF _eq.last_actuation_origin IS DISTINCT FROM 'local' THEN
    RETURN NEW;
  END IF;

  -- Nunca bloqueia manual (operador humano pode reassumir); bloqueia automation e polling redundante
  ...
  NEW.error_message := 'Atuação bloqueada — equipamento em modo Local (não interferir)';
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_guard_actuation_vs_local ON public.commands;
CREATE TRIGGER trg_guard_actuation_vs_local
BEFORE INSERT ON public.commands
FOR EACH ROW EXECUTE FUNCTION public.guard_actuation_command_vs_local();
