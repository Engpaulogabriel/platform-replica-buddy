-- Restaura comportamento: após 120s sem obediência, marca erro E enfileira TX 0 (desligar) por segurança.
...
BEGIN
  ...
  LOOP
    ...
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
            AND v_received_bit <> v_pending_expected_bit THEN
        -- Fora dos 120s e ainda não obedeceu: marca erro, enfileira TX 0 (desligar) por segurança
        v_next_desired_running := false;
        v_clear_pending := true;
        v_fail_pending := COALESCE(v_pending_status, 'pending') <> 'executed';
        v_enqueue_safety_off := true;
        v_blocked_until := now() + interval '30 seconds';
      ELSE
    ...
    IF v_fail_pending AND v_eq.pending_command_id IS NOT NULL THEN
      ...
      SET status = 'error',
          error_message = COALESCE(error_message, 'Bomba não obedeceu a ordem dentro da janela de 120s — TX 0 enfileirado por segurança'),
    ...
    -- Após marcar erro, dispara TX 0 físico para garantir bomba desligada
