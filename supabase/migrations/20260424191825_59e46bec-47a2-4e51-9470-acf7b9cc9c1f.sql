...
WITH stale AS (
  SELECT DISTINCT e.farm_id, e.id AS equipment_id
  ...
  WHERE e.type IN ('poco', 'bombeamento')
    ...
    AND c.frame ~ '\{1\}'
    AND (
      (e.last_outputs_state ~ '^[01]$' AND e.last_outputs_state = '0')
      OR (
        e.last_outputs_state ~ '^[01]{6}$'
        AND substring(e.last_outputs_state from COALESCE(e.saida, 1)::int for 1) = '0'
      )
    )
), cleared AS (
  ...
  WHERE e.id = s.equipment_id
    AND e.farm_id = s.farm_id
  RETURNING s.farm_id
)
UPDATE public.commands c
SET status = 'cancelled',
    responded_at = now(),
    error_message = 'Polling cancelado apos correcao de estado desejado'
WHERE c.type = 'polling'
  AND c.status = 'pending'
  AND c.farm_id IN (SELECT DISTINCT farm_id FROM cleared);