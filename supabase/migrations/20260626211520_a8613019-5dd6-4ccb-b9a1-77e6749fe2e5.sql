...
DECLARE
  ...
  _anon text := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6ImRueXVrZ2ZlZHJlZHZ4cHpqcHF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY2ODU1OTQsImV4cCI6MjA5MjI2MTU5NH0.OSg44w0CRVvD-f6Ts_U9DVeQkQ-4c37passKEK5X0kk';
...
BEGIN
  ...
  -- OFFLINE: proteção anti-flap rígida (15 min).
  -- Se o status foi flipado por qualquer mecanismo antes de 15 min sem RX real,
  -- não envia WhatsApp. O cron critical-alerts-tick é a fonte autoritativa.
  IF COALESCE(_alerts_enabled, false)
    ...
    IF NEW.last_communication IS NOT NULL
       AND NEW.last_communication < now() - interval '15 minutes' THEN
  ...
  -- BACK_ONLINE: só envia se a fase offline era real (15+ min sem RX antes da volta).
  -- Flap momentâneo que nunca deveria ter gerado OFFLINE não gera ONLINE de retorno.
  IF COALESCE(_alerts_enabled, false)
     AND OLD.communication_status = 'offline'
     AND NEW.communication_status = 'online' THEN
    IF OLD.last_communication IS NOT NULL
       AND OLD.last_communication < now() - interval '15 minutes' THEN
      PERFORM net.http_post(
        url := _url,
        headers := jsonb_build_object('Authorization', 'Bearer ' || _anon, 'apikey', _anon, 'Content-Type', 'application/json'),
