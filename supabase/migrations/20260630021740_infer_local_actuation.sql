CREATE OR REPLACE FUNCTION public.apply_pump_telemetry(_farm_id uuid, _tsnn text, _payload text, _signal_bars smallint, _command_id uuid, _raw_response text, _origin text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    -- Procura comando MANUAL recente (5 min) p/ este equipamento cujo bit do payload coincide com o bit recebido.
    -- Isso evita marcar 'local' quando a bomba está respondendo a um comando remoto recém-executado.
    ...
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF COALESCE(v_eq.last_actuation_origin, '') = 'local' AND NOT v_recent_safety_expiry THEN
        v_next_desired_running := v_new_running;
