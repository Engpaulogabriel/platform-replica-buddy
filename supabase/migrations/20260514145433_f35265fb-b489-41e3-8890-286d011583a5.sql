...
DECLARE
  ...
  post_lost_min_v int := 0;          -- min de atraso da PRIMEIRA bomba (referência)
  post_lost_pump_min_v int := 0;     -- min-bomba (somatório por bomba)
  post_late_pumps_v int := 0;        -- nº bombas com atraso > 5 min
  gap_min_v int := 0;                -- já é min-bomba (soma dos gaps de cada bomba)
  gap_pumps_v int := 0;
  lost_pump_min_v int := 0;          -- TOTAL min-bomba do dia
...
BEGIN
  ...
  -- Bombas que operaram no ciclo (turn_on registrado)
  ...
  -- Inclui bombas ligadas agora mesmo que ainda não tenham log
  IF pumps_running_now_v > pumps_operated_v THEN
    pumps_operated_v := pumps_running_now_v;
  END IF;

  -- Fallback: 1º ON do ciclo (referência)
  SELECT MIN(occurred_at) INTO post_on_fallback_v
    FROM automation_log
   WHERE farm_id = _farm_id
     AND action = 'turn_on'
     AND result = 'success'
  ...
  -- ── Atraso PÓS-PONTA por BOMBA (min-bomba) ──
  IF after_today_peak THEN
    ...
    per_pump AS (
      SELECT equipment_id, first_on,
             GREATEST(0, FLOOR(EXTRACT(epoch FROM (first_on - today_peak_end))/60))::int AS late_min
        FROM first_post_today
    )
    SELECT
      COALESCE(SUM(late_min), 0),
      COUNT(*) FILTER (WHERE late_min > 5),
      COALESCE(MAX(late_min), 0),
      COALESCE(MIN(first_on), post_on_fallback_v),
      COUNT(*) FILTER (WHERE first_on <= today_peak_end + interval '5 minutes')
    INTO post_lost_pump_min_v, post_late_pumps_v, post_lost_min_v, post_on_v, post_ok_v
    FROM per_pump;
  ELSE
    post_ok_v := CASE WHEN post_on_v IS NOT NULL THEN GREATEST(pumps_running_now_v, pumps_operated_v) ELSE 0 END;
  END IF;

  -- ── Bombas na ponta (já é min-bomba: SUM de minutos por bomba) ──
  IF in_or_after_today_peak THEN
  ...
  -- ── Paradas (gaps) no ciclo — JÁ é min-bomba ──
  WITH pump_equipment AS (
  ...
  SELECT COALESCE(FLOOR(SUM(gap_minutes))::int, 0),
         COUNT(*) FILTER (WHERE gap_minutes >= 1)
    INTO gap_min_v, gap_pumps_v FROM pump_gaps;

  -- TOTAL min-bomba perdidos no ciclo
  lost_pump_min_v := COALESCE(post_lost_pump_min_v, 0)
                   + COALESCE(gap_min_v, 0)
                   + COALESCE(minutes_peak_v, 0);

  -- Capacidade do ciclo (min-bomba disponíveis fora da ponta = 21h = 1260 min)
  ...
  SELECT COALESCE(SUM(lost_minutes), 0)::int INTO lost_pump_min_7d_v
    FROM public.energy_efficiency_daily
   WHERE farm_id = _farm_id AND date >= today_local - 7 AND date < today_local;
  SELECT COALESCE(SUM(lost_minutes), 0)::int INTO lost_pump_min_30d_v
    FROM public.energy_efficiency_daily
