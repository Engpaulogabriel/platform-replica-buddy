-- Adiciona campos de impacto ao RPC get_energy_efficiency_summary:
-- last_post_peak_on: última bomba que ligou após 21h
-- pre_late_pumps: bombas que desligaram após 18:00 (tarifa 3x)
-- pre_avg_late: atraso médio de desligamento após 18:00
-- pre_last_off: último desligamento antes de 21h
...
BEGIN
  ...
  SELECT COALESCE(SUM(pp.late_min), 0)::int,
    ...
    INTO post_lost_pump_min_v, post_late_pumps_v, post_lost_min_v, post_on_v, post_last_on_v,
         post_ok_v, post_pumps_seen_v, gap_min_v, gap_pumps_v
  ...
  IF in_or_after_today_peak THEN
    -- Pré-ponta: último desligamento antes da ponta e bombas que estavam ligadas na ponta
    ...
    -- Bombas que ficaram ligadas durante a ponta = atraso pré-ponta
    ...
    SELECT COUNT(DISTINCT equipment_id),
           ...
           MAX(peak_off)
      INTO pre_late_pumps_v, pre_avg_late_v, pre_last_off_v
      FROM peak_sessions;
  END IF;
