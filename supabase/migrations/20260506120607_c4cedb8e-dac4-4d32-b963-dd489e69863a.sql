...
BEGIN
  ...
  -- Inclui poco/bombeamento/nivel para participar do rodizio.
  -- IMPORTANTE: ao escolher o output_count do PLC, considera-se apenas
  -- pg.output_count (default 1). 'saida' de equipamento tipo 'nivel' eh
  -- indice interno do sensor (N1/N2), NUNCA representa saida fisica.
  SELECT
  ...
  -- v_max_saida considera APENAS poco/bombeamento (saidas fisicas reais).
  -- Equipamentos de nivel ficam fora deste calculo para nao inflar o payload.
  ...
  -- Se a PLC so tem sensor de nivel cadastrado (sem bomba), v_max_saida=0:
  -- usa output_count do plc_groups (default 1) — payload sera '0' (so leitura).
  ...
  -- Se nao houver bomba/poço cadastrado nessa PLC, associa o comando ao
  -- primeiro equipamento de nivel para fins de rastreabilidade.
