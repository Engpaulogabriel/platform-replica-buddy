...
CREATE TABLE public.inema_daily_compliance (
  ...
  farm_id uuid NOT NULL, equipment_id uuid NOT NULL, equipment_name text, day date NOT NULL,
  hours numeric, hours_limit numeric, hours_pct numeric,
  volume_m3 numeric, volume_limit numeric, volume_pct numeric,
  volume_source text, peak_pct numeric, status text,
  alerted boolean NOT NULL DEFAULT false, updated_at timestamptz NOT NULL DEFAULT now(),
...
BEGIN
  SELECT farm_id INTO v_farm FROM public.inema_daily_compliance WHERE equipment_id = _equipment_id AND day = v_d;
  IF v_farm IS NULL THEN RETURN; END IF;
  IF NOT public.has_farm_access(auth.uid(), v_farm) THEN RAISE EXCEPTION 'Sem permissao'; END IF;
  UPDATE public.inema_daily_compliance SET alerted = true WHERE equipment_id = _equipment_id AND day = v_d;
END $fn$;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT count(*), count(*) FILTER (WHERE peak_pct < 100), count(*) FILTER (WHERE peak_pct >= 100),
         CASE WHEN count(*) > 0 THEN round(count(*) FILTER (WHERE peak_pct < 100)::numeric / count(*) * 100, 1) END
  FROM public.inema_daily_compliance
  WHERE farm_id = _farm_id AND day >= (now() AT TIME ZONE 'America/Bahia')::date - _days
    AND public.has_farm_access(auth.uid(), _farm_id);
$$;

GRANT EXECUTE ON FUNCTION public.inema_snapshot(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.inema_mark_alerted(uuid, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.inema_farm_score(uuid, int) TO authenticated;