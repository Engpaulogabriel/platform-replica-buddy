-- Tabela de roteamento RF por farm
CREATE TABLE IF NOT EXISTS public.rf_routing (
  farm_id uuid PRIMARY KEY REFERENCES public.farms(id) ON DELETE CASCADE,
  radio text NOT NULL DEFAULT 'R1' CHECK (radio IN ('R1','R2','R3')),
  via_repetidor boolean NOT NULL DEFAULT false,
  updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.rf_routing ENABLE ROW LEVEL SECURITY;

CREATE POLICY "rf_routing_select_members"
  ON public.rf_routing FOR SELECT TO authenticated
  USING (public.has_farm_access(auth.uid(), farm_id));

CREATE POLICY "rf_routing_insert_writers"
  ON public.rf_routing FOR INSERT TO authenticated
  WITH CHECK (public.can_write_farm(auth.uid(), farm_id));

CREATE POLICY "rf_routing_update_writers"
  ON public.rf_routing FOR UPDATE TO authenticated
  USING (public.can_write_farm(auth.uid(), farm_id))
  WITH CHECK (public.can_write_farm(auth.uid(), farm_id));

CREATE POLICY "rf_routing_delete_admins"
  ON public.rf_routing FOR DELETE TO authenticated
  USING (public.is_farm_admin(auth.uid(), farm_id));

CREATE TRIGGER rf_routing_touch_updated_at
  BEFORE UPDATE ON public.rf_routing
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();

-- Atualiza a função run_automation_tick para usar o mesmo frame da web
...
BEGIN
  ...
  LOOP
    ...
    -- Carrega roteamento RF da fazenda (default R1 / direto)
    ...
    -- Decide TSNN (igual ao commandQueue.ts da web)
    ...
    THEN
      ...
      IF FOUND THEN
        v_payload := '1';
        -- Frame igual ao manual da web: buildLoRaFrame + roteamento opcional
