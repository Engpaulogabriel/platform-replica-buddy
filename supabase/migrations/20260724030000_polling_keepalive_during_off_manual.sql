-- v3.25.25 — Keep-alive polling durante comando manual de DESLIGAR.
--
-- PROBLEMA: enqueue_polling_for_due_equipments_internal excluía uma PLC do polling
-- por 90s sempre que ela tinha um comando manual pending/sent. Numa bomba ligada
-- localmente que recebe um OFF que nunca confirma (a bomba fica ON pela botoeira),
-- o comando fica ativo até ~120s (safety) e a PLC INTEIRA fica cega — sem keep-alive
-- e sem detecção — por todo esse tempo. Numa fazenda de 29 bombas isso paralisa a
-- comunicação de todas as bombas daquela PLC.
--
-- CONTEXTO: a partir do agente v3.25.24 o polling é SEMPRE {0} (keep-alive passivo,
-- forcePollingKeepAliveZero) e NUNCA atua o relé ({1} só existe na sequência manual
-- de desligamento forçado). Logo, um polling {0} não pode "contrariar" um comando de
-- DESLIGAR — ao contrário, reforça o {0} e mantém o INO vivo (watchdog 15min).
--
-- FIX: a exclusão de polling só se aplica a manuais que LIGAM alguma saída (payload
-- contém '1'), onde um {0} de polling poderia contrariar. Para manuais de DESLIGAR
-- (payload só {0}) a PLC continua recebendo keep-alive normalmente. Nenhuma outra
-- lógica muda.
--
-- Regex: c.frame ~ '\{[01]*1' casa apenas quando há um '1' dentro do payload {..};
-- '{0}'/'{000}' não casam (a barra/chave só aparece no grupo de payload, os
-- delimitadores [TSNN_1_] usam colchetes).
...
BEGIN
  ...
  WHERE e.farm_id = _farm_id
    ...
    AND NOT EXISTS (
      ...
      WHERE c.farm_id = _farm_id
        ...
        -- v3.25.25: só bloqueia keep-alive se o manual LIGA alguma saída ({..1..}).
        -- Manual de DESLIGAR (payload só {0}) não bloqueia — polling {0} é keep-alive
        -- passivo e reforça o próprio OFF.
        AND c.frame ~ '\{[01]*1'
  ...
  -- BUG FIX: SEMPRE enviar {payload} com 0 ou 1. NUNCA {} vazio.
  -- Protocolo Renov: PLC não responde a payload vazio. O polling sempre
  -- espelha o desired_running (mesmo se coincidir com o estado real).
  -- (O agente v3.25.24+ zera este payload para {0} antes do TX — keep-alive
  -- passivo — mas mantemos o desired aqui para compatibilidade da RPC.)
