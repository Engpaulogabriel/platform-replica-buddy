...
BEGIN
  ...
  -- IMPORTANTE: sempre limpa pending_command_id quando recebemos a resposta
  -- daquele exato comando (mesmo que o estado lido divirja do esperado).
  -- Isso libera a UI do "Ligando.../Desligando..." imediatamente.
  -- Se o PLC nao obedeceu, o cliente ja marcou v_origin='local' e bloqueou 30s.
...
-- Limpa o pending preso da bomba TESTE (estado atual em produção)
UPDATE public.equipments
SET pending_command_id = NULL, updated_at = now()
WHERE pending_command_id IS NOT NULL
  AND id IN (
    SELECT e.id FROM public.equipments e
    LEFT JOIN public.commands c ON c.id = e.pending_command_id
    WHERE c.responded_at IS NOT NULL
       OR c.status IN ('executed','timeout','error','cancelled')
  );