DROP FUNCTION IF EXISTS public.apply_pump_telemetry(uuid, text, text, smallint, uuid, text, text);

CREATE OR REPLACE FUNCTION public.apply_pump_telemetry(_farm_id uuid, _tsnn text, _payload text, _signal_bars smallint, _command_id uuid, _raw_response text, _origin text DEFAULT NULL)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
