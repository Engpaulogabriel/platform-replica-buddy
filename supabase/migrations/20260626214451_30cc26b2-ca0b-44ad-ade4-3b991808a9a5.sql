...
ALTER TABLE public.equipments ADD COLUMN IF NOT EXISTS telemetry_interval INTEGER NOT NULL DEFAULT 10;
UPDATE public.equipments SET telemetry_interval = 25 WHERE name ILIKE '%booster%';
...
BEGIN
  ...
  -- Janela de offline por equipamento: telemetry_interval + 5 min.
  -- Padrão = 10 (→ 15 min). Boosters = 25 (→ 30 min).
  _offline_min := COALESCE(NEW.telemetry_interval, 10) + 5;
