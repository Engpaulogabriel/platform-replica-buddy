-- Polling sequencial estrito: 1 PLC por tick, ordem ASC por hw_id da PLC
...
BEGIN
  ...
  -- Limpa pollings antigos com payload vazio (legado)
  ...
  -- IMPORTANTE: enfileira UMA PLC por chamada, sempre na ordem ASCENDENTE de hw_id da PLC.
  -- Combinado com o scheduler de 8s no cliente, gera ciclo sequencial estrito:
  -- PLC mais antiga (menor hw_id) -> próxima -> ... -> volta ao início.
  FOR v_grp IN
    WITH due AS (
      SELECT e.id,
             e.hw_id,
    ...
    SELECT *
    FROM grouped
    -- Ordem indiscutível: hw_id da PLC ASC. Em empate (mesma rodada), a que está esperando há mais tempo.
    ORDER BY tsnn ASC, oldest_polling ASC
    LIMIT 1
  LOOP
    -- Já tem polling pendente para essa PLC? Não duplica.
    IF EXISTS (
      SELECT 1
    ...
    FOR v_i IN 1..v_max_saida LOOP
      ...
      -- OVERRIDE: dentro de 60s após comando manual, polling reafirma intenção
      SELECT
        CASE
