...
BEGIN
  ...
  IF NOT COALESCE(_is_level_sensor, false)
    ...
    -- === NEW: enfileira notificação para comandos REMOTOS (plataforma web,
    -- WhatsApp já dispara imediato mas fica como fallback). Ignora 'automacao'
    -- (o drain suprime) e 'local' (já coberto acima).
    ...
    IF _is_web_remote THEN
      ...
      -- Anti-dup 5min: já existe pending_notifications ou log recente para o mesmo state?
      SELECT EXISTS (
        SELECT 1
        FROM public.pending_notifications pn
        WHERE pn.equipment_id = NEW.id
