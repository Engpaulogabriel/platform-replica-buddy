...
DECLARE
  ...
  v_frame text;
  v_command_id uuid;
  -- RESET aguarda a janela física completa (120s) pela resposta {0} da bomba.
  -- O Electron Agent já ignora RX divergente (ex: {1}) durante comando manual,
  -- então o reset fica esperando a confirmação correta de DESLIGADO até 120s.
...
BEGIN
  ...
  -- Payload POSICIONAL de OFF da saida alvo:
  --   saida 1 -> {0}, saida 2 -> {00}, saida 3 -> {000}, etc.
  -- O Electron compara o ultimo bit e ignora RX divergente ({1}) durante o comando.
...
-- Sobe timeout dos resets pendentes ja enfileirados para 120s
UPDATE public.commands
