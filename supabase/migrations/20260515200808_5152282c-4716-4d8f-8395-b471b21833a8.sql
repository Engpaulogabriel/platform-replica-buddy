CREATE TABLE IF NOT EXISTS public.automation_tick_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  executed_at timestamptz NOT NULL DEFAULT now(),
  schedules_found integer NOT NULL DEFAULT 0,
  commands_inserted integer NOT NULL DEFAULT 0,
  details jsonb NOT NULL DEFAULT '{}'::jsonb
);

ALTER TABLE public.automation_tick_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS automation_tick_logs_select_platform_staff ON public.automation_tick_logs;
CREATE POLICY automation_tick_logs_select_platform_staff
ON public.automation_tick_logs
FOR SELECT
TO authenticated
...
BEGIN
  ...
  LOOP
    ...
    IF NOT v_skip_day_check
       AND NOT (v_dow_key_pt = ANY(v_sched.days) OR v_dow_key_en = ANY(v_sched.days))
    THEN
      CONTINUE;
    END IF;
    ...
    THEN
      v_fired_key := v_sched.id::text || '_on_' || v_today_key;
      INSERT INTO public.automation_fired(schedule_id, fired_key, fired_at)
      VALUES (v_sched.id, v_fired_key, v_now) ON CONFLICT DO NOTHING;
      IF FOUND THEN
    ...
    IF v_sched.mode <> 'on-only'
       AND v_hhmm = v_effective_off
       AND v_sched.pending_command_id IS NULL
       AND v_currently_running = true
    THEN
      v_fired_key := v_sched.id::text || '_off_' || v_today_key;
      INSERT INTO public.automation_fired(schedule_id, fired_key, fired_at)
      VALUES (v_sched.id, v_fired_key, v_now) ON CONFLICT DO NOTHING;
      IF FOUND THEN
        v_payload := public.renov_combined_payload(
  ...
  VALUES (v_evaluated, v_enqueued, jsonb_build_object('mode', 'exact-minute-event', 'cron_status', 'left-disabled'));
