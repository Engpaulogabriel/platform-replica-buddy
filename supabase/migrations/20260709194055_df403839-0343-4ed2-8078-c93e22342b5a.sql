...
DECLARE
  ...
  cycle_end timestamptz;                    -- now()
  post_window_end timestamptz;              -- cycle_start + 9h (limite p/ cap 540)
  today_peak_start timestamptz;             -- 18:00 do dia atual do ciclo
  today_peak_end timestamptz;               -- 21:00 do dia atual do ciclo
  pre_peak_target timestamptz;              -- 17:45 (referência ideal de desligamento)
  ...
  -- Pós-ponta (novo cálculo)
  post_lost_pump_min_v int := 0;   -- soma de minutos atrasados
  post_lost_min_v int := 0;        -- máximo individual
  post_late_pumps_v int := 0;      -- bombas atrasadas (> 8 min)
  post_ok_v int := 0;              -- bombas dentro da tolerância (<= 8 min)
  post_pumps_seen_v int := 0;      -- total de bombas esperadas
  post_not_started_v int := 0;     -- não ligaram no pós-ponta (cap 540)
  post_on_v timestamptz;
  post_last_on_v timestamptz;

  -- Pré-ponta (novo cálculo com referência 17:45)
  pre_lost_pump_min_v int := 0;
  pre_early_off_pumps_v int := 0;      -- desligadas antes de 17:45
  pre_local_pumps_v int := 0;          -- em LOCAL desligadas cedo (flag manutenção)
...
BEGIN
  ...
  cycle_end := now();
  post_window_end := cycle_start + interval '9 hours';           -- 21:00 -> 06:00
  ...
  -- === Timeline de eventos (turn_on/turn_off) por bomba, com carry-on ===
  ...
  -- Bombas com programação ativa (universo esperado)
  scheduled_pumps AS (
  ...
  -- ====== PÓS-PONTA (novo): validação de 1h contínua ======
  -- Universo esperado = bombas com programação OR que apareceram operando
  expected_pumps AS (
    SELECT equipment_id FROM scheduled_pumps
    UNION
    SELECT equipment_id FROM operated
  ),
  -- Só sessões dentro da janela pós-ponta (21:00 -> 06:00 ou fim do ciclo)
  post_sessions AS (
    SELECT vs.equipment_id, vs.on_at, vs.off_at, vs.on_minutes,
           -- consideramos "válida" se durou 60min OU ainda está rodando (off_at = cycle_end)
           (vs.on_minutes >= 60 OR vs.off_at >= cycle_end) AS is_valid_entry
      FROM valid_sessions vs
     WHERE vs.on_at < LEAST(post_window_end, cycle_end)
       AND vs.off_at > cycle_start
  ),
  -- primeiro acionamento VÁLIDO por bomba
  first_valid_on AS (
    SELECT equipment_id, MIN(GREATEST(on_at, cycle_start)) AS first_on
      FROM post_sessions
     WHERE is_valid_entry = true
  ...
  post_per_pump AS (
    SELECT ep.equipment_id,
           fvo.first_on,
           CASE
             WHEN fvo.first_on IS NULL THEN 540      -- não ligou = cap 9h
             ELSE LEAST(540,
  ...
  SELECT COALESCE(SUM(pp.late_min), 0)::int,                                            -- soma total
         COUNT(*) FILTER (WHERE pp.late_min > 8)::int,                                  -- atrasadas (fora tolerância 8min)
         COALESCE(MAX(pp.late_min), 0)::int,                                            -- máximo individual
         MIN(pp.first_on),                                                              -- primeira a ligar
         MAX(pp.first_on),                                                              -- última a ligar
         COUNT(*) FILTER (WHERE pp.late_min <= 8 AND NOT pp.not_started)::int,          -- OK
         COUNT(*)::int,                                                                 -- total esperado
         COUNT(*) FILTER (WHERE pp.not_started)::int,                                   -- não ligaram
  ...
  -- pumps_operated = todas as bombas esperadas (mesmo que não tenham ligado)
  ...
  -- ====== PRÉ-PONTA (novo): referência 17:45 ======
  IF in_or_after_today_peak THEN
    -- Último desligamento agregado antes de 18:00
    ...
    -- Cálculo por bomba: encontrar último off antes de 18:00 e comparar a 17:45
    WITH pump_equipment AS (
    ...
    pre_calc AS (
      SELECT lop.equipment_id, lop.last_off,
             ...
             -- LOCAL se local_ack_at recente (últimas 24h) e no ciclo atual
    ...
    -- pre_ok = bombas que desligaram entre 17:45 e 18:00 (ok)
    ...
    -- Bombas ligadas durante a ponta (alerta crítico)
  ...
  -- Minutos de ponta violados (bombas que ligaram/ficaram ligadas 18:00-21:00)
  IF in_or_after_today_peak THEN
    WITH pump_equipment AS (
      SELECT id,
             CASE
