...
DECLARE
  ...
  v_dead_threshold    interval := interval '2 hours';
  v_tsnn text;
  v_frame text;
  v_payload text;
  v_total int;
...
BEGIN
  FOR v_eq IN
    ...
    WHERE e.active = true
      ...
      AND e.last_communication < now() - v_offline_threshold
      AND e.last_communication > now() - v_dead_threshold
      -- v3.11.7: se equipamento já está marcado como offline pela camada de
      -- monitoramento, NÃO enfileirar protective-off. O TSNN claramente não
      -- responde; despachar mais TX só atrapalha o rodízio dos poços sadios.
      AND COALESCE(e.communication_status, 'unknown') <> 'offline'
  LOOP
