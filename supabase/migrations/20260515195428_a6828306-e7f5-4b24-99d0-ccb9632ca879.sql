...
-- Limpeza: comandos cloud-automation antigos travados em 'sent' que já tiveram resposta (last_outputs_state confirmou)
UPDATE public.commands c
SET status = 'executed',
    responded_at = COALESCE(responded_at, now()),
    response = COALESCE(response, 'auto-resolved: estado já confirmado em equipments.last_outputs_state')
WHERE c.source_device = 'cloud-automation'
  AND c.status = 'sent'
  AND c.sent_at < now() - interval '3 minutes';

-- Limpa pending_command_id travado em equipamentos cujo comando associado já não está mais pendente
UPDATE public.equipments e
SET pending_command_id = NULL,
    updated_at = now()
WHERE e.pending_command_id IS NOT NULL
  AND EXISTS (
    SELECT 1 FROM public.commands c
    WHERE c.id = e.pending_command_id
