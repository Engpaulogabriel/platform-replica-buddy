-- v3.25.15: preservar last_actuation_origin='local' quando um POLLING/espontâneo
-- apenas CONFIRMA o estado que já bate com o desired_running (sem ser confirmação
-- de comando manual). Antes, um poll de reforço que confirmava o estado fazia a RPC
-- re-atribuir a origem para remote (via _origin='remote-desired' do agente e/ou o
-- ramo "current='local' AND desired_matches → v_origin='remote'"), apagando o badge
-- LOCAL. Opção B (safety net do fix de polling desnecessário no agente).
--
-- Mudanças vs 20260723150000:
--   1. Nova variável v_preserve_local.
--   2. v_preserve_local = (origem atual 'local') AND (received confirma o desired)
--      AND (NÃO é confirmação de comando manual pendente).
--   3. UPDATE: last_actuation_origin usa CASE — quando v_preserve_local, mantém
--      'local'; caso contrário, COALESCE(_origin, v_origin, e.last_actuation_origin)
--      como antes. Confirmação de comando manual (v_pending_confirms_expected) segue
--      virando remote normalmente.
...
BEGIN
  ...
  LOOP
    ...
    -- v3.25.15: preservar 'local' quando um poll/espontâneo apenas CONFIRMA o estado
    -- que já bate com o desired (received==desired) e a origem atual já é 'local',
    -- desde que NÃO seja confirmação de um comando manual (aí é atuação remota real).
