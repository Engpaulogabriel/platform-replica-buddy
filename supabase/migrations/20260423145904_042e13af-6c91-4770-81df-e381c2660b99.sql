-- Polling em modo "somente leitura" quando bomba está em atuação local
...
BEGIN
  ...
  LOOP
    ...
    -- Modo somente leitura: bomba em atuação local OU dentro da janela de bloqueio
    v_read_only := (v_eq.last_actuation_origin = 'local')
                   OR (v_eq.command_blocked_until IS NOT NULL AND v_eq.command_blocked_until > now());

    IF v_read_only THEN
      -- Frame vazio {} → apenas lê estado da bomba sem forçar saída
      v_frame := '[' || v_tsnn || '_1_]{}[' || v_tsnn || '_ETX_]' || E'\r';
    ELSE
      -- Reforça estado desejado (1 dígito da saída desta bomba)
