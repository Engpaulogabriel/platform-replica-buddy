...
DECLARE
  ...
  -- Origem normalizada vinda do agente: 'local' | 'remote' | NULL (não informado)
...
BEGIN
  ...
  -- Normaliza _origin recebido do Electron
  ...
  LOOP
    ...
    -- Prioridade:
    --  1) v_explicit_origin (informado pelo Electron) — manda sempre.
    --  2) Se NÃO informado: usa heurística APENAS para confirmar 'remote'
    --     em casos óbvios (pending bate). NUNCA marca 'local' implicitamente.
    -- ─────────────────────────────────────────────────────────────────────
    IF v_explicit_origin IS NOT NULL THEN
      v_origin := v_explicit_origin;
      IF v_explicit_origin = 'local' THEN
    ...
    ELSE
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
        ...
        v_origin := 'remote';
      ELSE
        -- Origem não informada e nenhuma evidência de remoto: PRESERVA o valor anterior.
        v_origin := NULL;
      END IF;
    END IF;
...
REVOKE ALL ON FUNCTION public.apply_pump_telemetry(uuid, text, text, smallint, uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.apply_pump_telemetry(uuid, text, text, smallint, uuid, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.apply_pump_telemetry(uuid, text, text, smallint, uuid, text, text) TO service_role;