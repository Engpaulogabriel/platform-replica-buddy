-- v3.25.14: correção do LOOP de reforço após atuação local.
--
-- BUG: quando o agente detecta atuação local (LOCAL OVERRIDE), ele grava
-- desired_running=estado real DIRETO na tabela equipments e logo em seguida chama
-- apply_pump_telemetry com _origin='local'. A RPC, porém, RE-APLICAVA o bit do
-- comando manual recente ao desired_running:
--
...
--
-- Como o lookup de 60s acha o comando remoto recente (mesmo já 'executed'), e o
-- RX local diverge do bit esperado, a RPC gravava desired_running de volta para o
-- valor do comando — DESFAZENDO o override local. Resultado: o polling (que
-- espelha desired_running) seguia mandando o payload antigo → loop.
--
-- FIX: esse ramo de re-aplicação só deve valer para NÃO-locais. Quando
-- _origin='local' (v_explicit_origin='local'), a RPC não re-aplica o comando —
-- preserva o desired_running que o agente acabou de gravar (via COALESCE(NULL, ...)).
-- Casos legítimos (comando enviado, RX intermediário divergente) têm origem
-- remote/null e continuam re-aplicando normalmente.
--
-- ÚNICA mudança vs 20260723140000: "+ AND v_explicit_origin IS DISTINCT FROM 'local'"
-- na condição do ramo de re-aplicação do v_next_desired_running.
...
BEGIN
  ...
  LOOP
    ...
    IF v_received_bit IN ('0','1') THEN
      -- v3.25.9: a distinção eco vs atuação local é por ESTADO DO COMANDO, não por
      -- tempo. Não há mais janela de "recent remote match": o eco de um comando remoto
      -- é capturado por v_pending_confirms_expected (comando EM VOO cujo bit este RX
      -- confirma → v_origin='remote', abaixo). Se NÃO existe comando não-confirmado
      -- (v_pending_command_active=false; cobre pending/sent/delivered), um espontâneo
      -- que muda o estado é LOCAL. v_recent_remote_match fica false e não suprime mais.
    ...
    -- v3.25.9: comando do operador EM VOO/NA FILA = status não-terminal
    -- (pending/sent/delivered). 'pending' cobre o caso de FILA (ligar N bombas em
    -- série: as que ainda não receberam TX ficam pending e não podem virar local).
    ...
    -- FIX: Quando _origin (parâmetro) é NULL, preservar o valor atual de last_actuation_origin
    -- Quando _origin tem valor explícito, usar ele. Caso contrário, aplicar v_origin classificado
    -- somente se ele existir; nunca sobrescrever com default 'remote'.
    IF _origin IS NULL THEN
      v_origin := NULL;  -- força preservação via COALESCE abaixo
    END IF;
    ...
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_pending_confirms_expected THEN
        ...
        v_clear_pending := false;
            v_enqueue_safety_off := false;
        v_blocked_until := now() + interval '30 seconds';
      -- v3.25.14: re-aplicar o bit do comando SÓ para NÃO-locais. Numa atuação
      -- local (v_explicit_origin='local'), NÃO re-aplicar — preservar o
      -- desired_running que o agente gravou (evita desfazer o override local → loop).
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
            AND v_received_bit <> v_pending_expected_bit
    ...
    SET
      ...
      last_actuation_origin = COALESCE(_origin, v_origin, e.last_actuation_origin),
