-- ============================================================
-- FIX 1: apply_pump_telemetry — não seguir estado físico contra desired_running
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- =========================================================
    -- CORREÇÃO CRÍTICA: nunca seguir o estado físico contra desired_running
    -- =========================================================
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_received_bit = '1' AND COALESCE(v_eq.desired_running, false) = false THEN
        -- BOMBA LIGADA LOCALMENTE: usuário/sistema NÃO pediu para ligar.
        -- Mantém desired_running=false para que polling envie {0} e force desligamento.
        v_next_desired_running := false;
      ELSIF v_received_bit = '0' AND COALESCE(v_eq.desired_running, false) = true
        ...
        -- BOMBA DESLIGADA LOCALMENTE: já passou janela de partida, aceita o desligamento real.
        v_next_desired_running := false;
      ELSE
        -- Sem conflito: segue estado físico
...
-- ============================================================
-- FIX 2: enqueue_polling_for_due_equipments — usar desired_running, não estado físico
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Prioridade 1: comando manual pendente
