-- Nova lógica pós/pré-ponta:
--   Pós-ponta = APENAS bombas que ligaram entre peak_end e peak_end+3h (21:00-00:00 do dia da abertura do ciclo).
--     late_min = first_on_post - cycle_start (peak_end do dia anterior). status: 'ok' se <=8min, senão 'late'.
--     Bombas sem startup nessa janela NÃO entram no cálculo (nem 'not_started', nem 540).
--   Pré-ponta = APENAS bombas que desligaram entre peak_start-2h e peak_start (16:00-18:00).
--     early_off_min = max(0, (peak_start - 15min) - last_off_pre). status: 'ok'/'early'.
--     Bombas que desligaram antes das 16:00 NÃO entram (operação normal).
--   Universo = APENAS bombas que operaram no ciclo (sem scheduled_for_cycle).
...
BEGIN
  ...
  -- Pós-ponta: primeiro turn_on na janela [cycle_start, cycle_start+3h]
  first_post_on AS (
    SELECT vs.equipment_id, MIN(vs.on_at) AS first_on
      FROM valid_sessions vs
     WHERE NOT is_free
       AND vs.on_at >= cycle_start
       AND vs.on_at < post_window_end
     GROUP BY vs.equipment_id
  ),
  -- Pré-ponta: último turn_off na janela [peak_start-2h, peak_start]
  last_off_pre AS (
    SELECT vs.equipment_id, MAX(vs.off_at) AS last_off
      FROM valid_sessions vs
     WHERE NOT is_free
       AND vs.off_at >= pre_window_start
       AND vs.off_at <= peak_start
     GROUP BY vs.equipment_id
  ),
  ...
  SELECT
    ...
    CASE
      ...
      WHEN fpo.first_on IS NULL THEN 'ok'  -- não ligou no pós-ponta → não penaliza
      WHEN GREATEST(0, FLOOR(EXTRACT(epoch FROM (fpo.first_on - cycle_start))/60))::int > 8 THEN 'late'
      ELSE 'ok'
    END AS post_status,
    CASE
      WHEN is_free THEN 'ok'
      WHEN lop.last_off IS NULL THEN 'ok'  -- não desligou perto da ponta → não penaliza
...
BEGIN
  ...
  first_post_on AS (
    SELECT vs.equipment_id, MIN(vs.on_at) AS first_on
      FROM valid_sessions vs
     WHERE vs.on_at >= cycle_start AND vs.on_at < post_window_end
     GROUP BY vs.equipment_id
  ),
  post_per_pump AS (
    SELECT cu.equipment_id, fpo.first_on,
           CASE WHEN fpo.first_on IS NULL THEN 0
                ELSE GREATEST(0, FLOOR(EXTRACT(epoch FROM (fpo.first_on - cycle_start))/60))::int
           END AS late_min
      FROM operated_in_cycle cu
      LEFT JOIN first_post_on fpo ON fpo.equipment_id = cu.equipment_id
  ),
  last_off_pre AS (
    SELECT vs.equipment_id, MAX(vs.off_at) AS last_off
      FROM valid_sessions vs
     WHERE vs.off_at >= pre_window_start AND vs.off_at <= peak_start
     GROUP BY vs.equipment_id
  ),
  pump_span AS (
    SELECT equipment_id, MIN(on_at) AS first_on, MAX(off_at) AS last_off
  ...
  peak_per_pump AS (
    SELECT h.equipment_id, h.peak_minutes::numeric AS peak_minutes
      FROM public.calculate_pump_peak_minutes_for_window(_farm_id, peak_start, effective_peak_end) h
     WHERE effective_peak_end > peak_start
  ),
  metrics AS (
    SELECT
      COUNT(DISTINCT oi.equipment_id)::int AS pumps_operated_v,
      (SELECT MIN(first_on) FROM first_post_on) AS post_on_v,
      COALESCE((SELECT SUM(late_min) FROM post_per_pump), 0)::int AS post_lost_pump_min_v,
      COALESCE((SELECT FLOOR(SUM(gap_minutes))::int FROM pump_gaps), 0) AS gap_min_v,
      ...
      (SELECT MAX(last_off) FROM last_off_pre) AS pre_off_v,
      COALESCE((SELECT COUNT(*)::int FROM last_off_pre
                WHERE last_off >= peak_start - interval '15 minutes'), 0) AS pre_ok_v,
...
-- Ajustar summary: alinha pós-ponta com a nova regra (janela 21:00-00:00, sem "não ligou").
...
-- Recalcular últimos 30 dias para todas as fazendas com a nova lógica
DO $$
DECLARE
  f RECORD;
  d DATE;
