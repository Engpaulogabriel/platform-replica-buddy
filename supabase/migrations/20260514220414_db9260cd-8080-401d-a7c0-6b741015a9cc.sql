...
BEGIN
  ...
  -- Se nenhuma bomba ligada, captação é ZERO independente do sensor
  IF v_active_pumps = 0 THEN
    v_status := 'parada';
    v_prediction_h := NULL;
  ELSIF v_sensors_with_data = 0 OR v_avg_rate IS NULL THEN
    v_status := 'sem_dados';
    v_prediction_h := NULL;
  ...
  RETURN jsonb_build_object(
    ...
    'rate_per_hour', CASE WHEN v_status = 'parada' THEN 0 ELSE round(coalesce(v_avg_rate,0)::numeric * 60, 2) END,
    'sensor_rate_per_hour', round(coalesce(v_avg_rate,0)::numeric * 60, 2),
    'status', v_status,
