...
BEGIN
  ...
  -- FIX: Protocolo Renov exige payload SEMPRE com 6 caracteres (1 por saída
  -- física do PLC). Saídas não cadastradas no painel vão como '0'.
  v_total_saidas := 6;
  v_payload := '000000';
  ...
  FOR v_i IN 1..v_total_saidas LOOP
    SELECT
      CASE
        WHEN e.last_outputs_state ~ '^[01]{6}$'
    ...
    SELECT
      CASE
        WHEN c.frame ~ '\{[01]{6}\}'
          THEN substring(substring(c.frame from '\{([01]{6})\}') from v_i for 1)
        WHEN c.frame ~ '\{[01]\}' THEN substring(c.frame from '\{([01])\}')
        WHEN c.frame ~ '\{[01]{2,5}\}' AND length(substring(c.frame from '\{([01]{2,5})\}')) >= v_i
          THEN substring(substring(c.frame from '\{([01]{2,5})\}') from v_i for 1)
        ELSE NULL
      END
    INTO v_intent_bit
    FROM public.commands c
