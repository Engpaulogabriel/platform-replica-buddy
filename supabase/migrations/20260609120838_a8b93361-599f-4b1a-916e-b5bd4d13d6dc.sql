...
BEGIN
  FOR v_eq IN
    ...
    WHERE e.active = true
      ...
      -- v3.11.3: equipamento "morto" (>2h sem RX) — não adianta enfileirar
      -- mais RESETs; o rádio claramente não responde. Continua tentando
      -- religar via polling normal, mas sem inundar a fila com TX OFF que
      -- só servem para travar 120s o agente esperando confirmação.
