...
BEGIN
  ...
  LOOP
    ...
    IF v_state_changed THEN
      ...
      -- NOVO: Bomba estava LIGADA e DESLIGOU sozinha (origem local detectada).
      -- Forca envio de TX 0 com prioridade maxima para travar o rele em desligado.
      IF v_origin = 'local'
         AND v_old_running = true
         AND v_new_running = false THEN
        v_should_force_off := true;
      END IF;
    ELSE
      -- Sem mudanca de estado: se o comando manual pendente confirmou o estado esperado,
      -- limpa o icone "LOCAL" marcando origem como 'remote'.
      IF v_pending_confirms_expected THEN
        v_origin := 'remote';
      END IF;

      -- Se o comando manual era LIGAR (bit esperado=1) e a janela de 60s
      -- ja expirou e a telemetria continua com bit 0 (bomba nao obedeceu),
      -- marca como 'local' para acender o icone de alerta no Dashboard.
      IF v_pending_is_manual
         AND v_pending_expected_bit = '1'
         AND v_received_bit = '0'
         AND v_pending_started_at IS NOT NULL
         AND v_pending_started_at <= now() - interval '60 seconds' THEN
        v_origin := 'local';
        v_blocked_until := now() + interval '30 seconds';
      END IF;
    END IF;
    ...
    -- NOVO: Apos persistir telemetria, dispara TX 0 forcado se bomba desligou sozinha
    IF v_should_force_off THEN
      ...
      VALUES (
        ...
        format('Bomba %s desligou localmente enquanto estava ligada — TX 0 enfileirado com prioridade maxima para travar rele.', v_eq.id)
      );
    END IF;
  END LOOP;
