...
BEGIN
  ...
  LOOP
    ...
    IF v_pending_is_manual AND v_payload_safe IS NOT NULL AND v_pending_frame IS NOT NULL THEN
      ...
      -- ─── NOVA REGRA (pedido do usuário) ──────────────────────────────────
      -- Dentro dos 60s após disparar um comando manual, IGNORAMOS QUALQUER
      -- resposta da bomba (mesmo que confirme o estado esperado). O comando
      -- segue ativo, o polling continua reafirmando a intenção, e só após o
      -- minuto completo é que a próxima telemetria decide se houve sucesso
      -- ou falha. Isso evita "encerrar cedo" o estado Ligando…/Desligando…
      -- por um eco antigo da bomba e dá tempo físico real para o relé atuar.
    ...
    -- ─── PRESERVA O BIT DA SAÍDA DESTE EQUIPAMENTO DENTRO DA JANELA ────────
    -- Dentro da janela de 60s, mantemos o bit antigo no last_outputs_state
    -- (UI continua "Ligando…" / "Desligando…") independente do que a bomba
    -- ecoou. Os demais bits do payload (outras saídas do mesmo PLC) ficam
    -- atualizados normalmente.
    IF v_pending_within_start_window
       AND v_payload_safe IS NOT NULL
       AND v_eq.saida BETWEEN 1 AND 6 THEN
      IF v_eq.last_outputs_state ~ '^[01]{6}$' THEN
        v_old_state_padded := v_eq.last_outputs_state;
      ELSIF v_eq.last_outputs_state ~ '^[01]$' AND v_eq.saida = 1 THEN
        v_old_state_padded := rpad(v_eq.last_outputs_state, 6, '0');
      ELSE
        v_old_state_padded := '000000';
      END IF;

      v_payload_to_store := overlay(
        v_payload_safe
        placing substring(v_old_state_padded from v_eq.saida::int for 1)
        from v_eq.saida::int
        for 1
      );

      -- Sem mudança real para esta saída neste tick.
      v_received_bit := substring(v_payload_to_store from v_eq.saida::int for 1);
      v_new_running := v_received_bit = '1';
      v_state_changed := false;
      -- Forçamos: não considerar como confirmação dentro da janela.
      v_pending_confirms_expected := false;
    END IF;
    ...
    IF v_received_bit IN ('0', '1') THEN
      IF v_pending_within_start_window THEN
        -- Dentro da janela: mantém a intenção do comando como desired_running.
    ...
    -- Marca falha do comando pendente quando aplicável
    ...
    -- Confirmação remota: se a janela já fechou e a telemetria bate com a intenção,
    -- marca o comando como executado (caso ainda esteja pendente).
    ...
    -- Força desligamento protetivo (somente fora da janela)
