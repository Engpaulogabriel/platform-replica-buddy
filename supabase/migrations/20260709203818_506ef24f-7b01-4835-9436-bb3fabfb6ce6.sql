...
-- =====================================================================
-- get_energy_efficiency_summary: restringe universo pós-ponta a bombas
-- com programação ativa para o ciclo (time_on fora da janela de ponta).
-- =====================================================================
...
BEGIN
  ...
  scheduled_for_cycle AS (
    -- Bombas com programação ativa para ligar fora da janela de ponta
    -- (ou seja, programadas para o ciclo pós-ponta atual).
    SELECT DISTINCT s.equipment_id
     ...
     WHERE s.farm_id = _farm_id AND s.active = true AND s.equipment_id IS NOT NULL
       AND (s.time_on IS NULL
            OR s.time_on >= peak_end_local
            OR s.time_on < peak_start_local)
  ),
  raw_events AS (
  ...
  -- Universo do pós-ponta: apenas bombas com programação ativa para o ciclo.
  post_universe AS (
    SELECT ep.id AS equipment_id
      FROM pump_equipment ep
     WHERE ep.id IN (SELECT equipment_id FROM scheduled_for_cycle)
  ),
  post_per_pump AS (
    SELECT pu.equipment_id, fvo.first_on,
      ...
      FROM post_universe pu
      LEFT JOIN first_valid_on fvo ON fvo.equipment_id = pu.equipment_id
  ),
...
-- =====================================================================
-- calculate_energy_efficiency_pumps_for_date: filtra o snapshot por
-- bomba para incluir apenas equipamentos com programação ativa fora da
-- janela de ponta (evita 540 min em bombas sem programação).
-- =====================================================================
...
BEGIN
  ...
  -- Universo do pós-ponta: apenas bombas com programação ativa para o ciclo.
  post_universe AS (
    SELECT ep.id AS equipment_id FROM pump_equipment ep
     WHERE ep.id IN (SELECT equipment_id FROM scheduled_pumps)
  )
  SELECT
    ep.equipment_id,
