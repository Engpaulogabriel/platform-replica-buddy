-- v3.25.8: distinguir "eco de comando remoto" de "nova atuação local após o comando executado".
--
-- Problema: v_recent_remote_match usava janela de 5 min e casava QUALQUER comando
-- manual recente com o mesmo bit. Assim, uma bomba ligada local → desligada pela
-- plataforma (OFF confirmado) → religada pela botoeira (espontâneo bit=1) tinha o
-- religamento suprimido, porque um comando ON de até 5 min atrás (ou o próprio ciclo)
-- ainda "casava" o bit. O religamento é uma NOVA atuação local, não eco do comando.
--
-- Correção: v_recent_remote_match só conta como eco se o comando manual MAIS RECENTE
-- tem bit alvo == bit recebido E está dentro de 30s desde sua confirmação/envio.
-- Eco de RF chega em segundos (serial responde < 5s), então 30s é folgado; um comando
-- já concluído e superado por outro (ou antigo) não mascara mais o religamento local.
--
-- ÚNICA mudança em relação a 20260714051458: o bloco que computa v_recent_remote_match.
-- Restante da função idêntico.
...
BEGIN
  ...
  LOOP
    ...
    IF v_received_bit IN ('0','1') THEN
      -- v3.25.8: ECO vs NOVA ATUAÇÃO LOCAL.
      -- Só conta como eco de comando remoto se o comando manual MAIS RECENTE do
      -- equipamento tem bit alvo == bit recebido E está dentro de 30s desde sua
      -- confirmação/envio. Assim, um comando já concluído (ex.: OFF), ou um comando
      -- antigo, não mascara um religamento local subsequente com o mesmo bit.
      SELECT EXISTS (
        SELECT 1
        FROM (
          SELECT c.frame,
                 COALESCE(c.responded_at, c.sent_at, c.created_at) AS c_ts
          FROM public.commands c
          WHERE c.farm_id = _farm_id
            AND c.equipment_id = v_eq.id
            AND c.type = 'manual'
        ...
        WHERE lm.c_ts > now() - interval '30 seconds'
          AND (
            substring(lm.frame from '\{([01])\}') = v_received_bit
            OR (lm.frame ~ '\{[01]{2,6}\}'
                AND substring(substring(lm.frame from '\{([01]{2,6})\}')
                              from length(substring(lm.frame from '\{([01]{2,6})\}')) for 1) = v_received_bit)
          )
