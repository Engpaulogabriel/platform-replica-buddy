...
GRANT ALL ON public.environmental_agencies TO service_role;

ALTER TABLE public.environmental_agencies ENABLE ROW LEVEL SECURITY;

CREATE POLICY "env_agencies_select" ON public.environmental_agencies FOR SELECT TO authenticated USING (true);
...
UPDATE public.farms SET state_code = state WHERE state ~ '^[A-Z]{2}$' AND state_code IS NULL;