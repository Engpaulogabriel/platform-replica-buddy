...
DECLARE
  ...
  prev_peak_start timestamptz;
  prev_peak_end timestamptz;
...
BEGIN
  ...
  cycle_start_local := (today_local - 1)::timestamp + time '21:00';
  cycle_start := cycle_start_local AT TIME ZONE tz;
  cycle_end := now();
  prev_peak_start := ((today_local - 1)::text || ' 18:00')::timestamp AT TIME ZONE tz;
  prev_peak_end := ((today_local - 1)::text || ' 21:00')::timestamp AT TIME ZONE tz;
  ...
  IF post_on_v IS NULL AND pumps_running_now_v > 0 THEN
    post_on_v := cycle_start;
  END IF;
