...
-- 1) Trigger BEFORE INSERT em commands: bloqueia manual sem created_by
-- exceto fontes internas conhecidas
...
BEGIN
  IF NEW.type = 'manual' AND NEW.created_by IS NULL THEN
    ...
    ) AND COALESCE(NEW.source_device, '') NOT LIKE 'backend-reset:%' THEN
      RAISE EXCEPTION
...
DROP TRIGGER IF EXISTS trg_guard_manual_command_without_user ON public.commands;
...
-- 2) Recria apply_pump_telemetry adicionando log de auditoria
-- toda vez que desired_running é alterado por confirmação de comando manual
CREATE OR REPLACE FUNCTION public.apply_pump_telemetry(
  _farm_id uuid, _tsnn text, _payload text,
...
BEGIN
  ...
  LOOP
    ...
    -- ──────────────────────────────────────────────────────────────────
    -- AUDITORIA: log toda vez que desired_running será setado para TRUE
    -- (transição false→true ou reafirmação). Permite rastrear quem
    -- disparou um "Ligar" inesperado.
    -- ──────────────────────────────────────────────────────────────────
    IF v_next_desired_running IS TRUE AND v_old_desired IS DISTINCT FROM TRUE THEN
      SELECT id, type, source_device, created_by, created_at, frame
        INTO v_audit_cmd
      FROM public.commands
      WHERE id = COALESCE(v_eq.pending_command_id, _command_id)
      LIMIT 1;

      INSERT INTO public.agent_logs (farm_id, level, category, message, raw_frame)
      VALUES (
        _farm_id,
        'warn',
        'safety',
        format(
          'AUDIT desired_running=TRUE em eq %s (%s). cmd_id=%s type=%s source=%s created_by=%s expected_bit=%s received_bit=%s rx_payload=%s pending_frame=%s',
          v_eq.id, v_eq.name,
          COALESCE(v_audit_cmd.id::text, '<sem-cmd>'),
          COALESCE(v_audit_cmd.type::text, '?'),
          COALESCE(v_audit_cmd.source_device, '<null>'),
          COALESCE(v_audit_cmd.created_by::text, '<null>'),
          COALESCE(v_pending_expected_bit, '?'),
          COALESCE(v_received_bit, '?'),
          COALESCE(_payload, '<null>'),
          COALESCE(v_pending_frame, '<null>')
        ),
        _raw_response
      );
    END IF;
