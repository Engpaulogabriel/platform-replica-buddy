...
SET desired_running = CASE
  WHEN last_outputs_state ~ '^[01]{6}$' AND COALESCE(saida, 1) BETWEEN 1 AND 6
    THEN substring(last_outputs_state from COALESCE(saida, 1)::int for 1) = '1'
  WHEN last_outputs_state ~ '^[01]$'
    THEN last_outputs_state = '1'
  ELSE false
END;
...
BEGIN
  IF NEW.type = 'manual' AND NEW.equipment_id IS NOT NULL THEN
    ...
    v_payload := substring(NEW.frame from '\{([01]{1,6})\}');

    IF v_payload ~ '^[01]$' THEN
      v_desired := v_payload = '1';
    ELSIF v_payload ~ '^[01]{2,6}$' AND length(v_payload) >= v_saida THEN
      v_desired := substring(v_payload from v_saida for 1) = '1';
    END IF;

    UPDATE public.equipments
    SET pending_command_id = NEW.id,
        desired_running = COALESCE(v_desired, desired_running),
        updated_at = now()
    WHERE id = NEW.equipment_id
      AND farm_id = NEW.farm_id;
...
BEGIN
  ...
  LOOP
    ...
    IF v_received_bit IN ('0', '1') THEN
      IF v_eq.pending_command_id IS NOT NULL AND v_pending_is_manual AND NOT v_pending_confirms_expected THEN
