-- Guard: backend must NEVER mark a manual command as 'error' before the Electron
-- has sent the frame (sent_at IS NULL) or before the 120s reinforcement window
-- has elapsed. Marking commands as error/executed is Electron's responsibility.
-- We narrow the two UPDATEs inside apply_pump_telemetry (fail branch + safety
-- reset enqueue) so they only fire when sent_at IS NOT NULL AND sent_at is
...
DECLARE
  ...
  v_pending_sent_at timestamptz;  -- v3.15.1: raw sent_at (not COALESCEd) to gate error marking
  v_pending_reinforcement_expired boolean := false;
...
BEGIN
  v_explicit_origin := _origin;
  v_tsnn_norm := lpad(regexp_replace(COALESCE(_tsnn,''), '\D', '', 'g'), 4, '0');

  IF _command_id IS NOT NULL THEN
    SELECT equipment_id, frame, type = 'manual'
      INTO v_cmd_equipment_id, v_cmd_frame, v_command_is_manual
  ...
  FOR v_eq IN
    SELECT e.id, e.saida, e.desired_running, e.last_outputs_state,
           e.pending_command_id, e.last_actuation_origin, e.command_blocked_until,
           e.tsnn
    FROM public.equipments e
    WHERE e.farm_id = _farm_id
      AND lpad(regexp_replace(COALESCE(e.tsnn,''), '\D', '', 'g'), 4, '0') = v_tsnn_norm
    ORDER BY e.saida NULLS LAST
  LOOP
    IF v_first_eq_id IS NULL THEN v_first_eq_id := v_eq.id; END IF;

    v_base_state := COALESCE(v_eq.last_outputs_state, '000000');
    IF length(v_base_state) < 6 THEN v_base_state := lpad(v_base_state, 6, '0'); END IF;

    v_is_full_bitfield := (_payload ~ '^[01]{6}$');
    IF v_is_full_bitfield THEN
      ...
      END IF;
    ELSIF _payload ~ '^[01]{1}$' THEN
      IF v_eq.saida BETWEEN 1 AND 6 THEN
        v_payload_to_store := overlay(v_base_state placing _payload from v_eq.saida::int for 1);
        v_received_bit := _payload;
      ELSE
        v_payload_to_store := v_base_state;
      END IF;
    ELSIF _payload ~ '^[01]{2,5}$' THEN
      v_payload_to_store := v_base_state;
      IF v_eq.saida BETWEEN 1 AND length(_payload) THEN
        v_received_bit := substring(_payload from v_eq.saida::int for 1);
      END IF;
    ELSE
      v_payload_to_store := v_base_state;
    ...
    IF v_eq.pending_command_id IS NOT NULL THEN
      SELECT frame, type = 'manual', status, COALESCE(sent_at, created_at), source_device, sent_at
        INTO v_pending_frame, v_pending_is_manual, v_pending_status, v_pending_started_at, v_pending_source_device, v_pending_sent_at
      FROM public.commands
    ...
    -- v3.15.1: reinforcement window guard. Backend só pode marcar comando manual
    -- como error se (a) o Electron JÁ enviou o frame (sent_at IS NOT NULL) E
    -- (b) já passaram 120s do envio (janela de reforço do Electron expirou).
    v_pending_reinforcement_expired := v_pending_sent_at IS NOT NULL
                                        AND v_pending_sent_at < now() - interval '120 seconds';
    ...
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
        ...
        -- v3.15.1: só pode falhar/limpar/agendar safety-off se o Electron JÁ enviou o frame
        -- E o timeout de reforço (120s) expirou. Enquanto sent_at IS NULL, o Electron
        -- nem começou a atuar — a divergência é normal (bomba ainda desligada antes do LIGAR).
        IF v_pending_reinforcement_expired THEN
          v_next_desired_running := NULL;
          v_clear_pending := true;
          ...
          v_blocked_until := now() + interval '30 seconds';
        ELSE
          -- Reforço ainda ativo (ou frame ainda não enviado): NÃO tocar no comando.
          -- Responsabilidade de marcar error/executed é EXCLUSIVA do Electron.
          v_next_desired_running := NULL;
          v_clear_pending := false;
          v_fail_pending := false;
          v_enqueue_safety_off := false;
    ...
    IF v_state_changed AND v_new_running IS NOT NULL THEN
      INSERT INTO public.automation_log (
        farm_id, equipment_id, event_type, origin, result, reason,
        state_after, source_device, executed_at, details
      ) VALUES (
        _farm_id, v_eq.id,
        CASE WHEN v_new_running THEN 'pump_on'::public.event_type ELSE 'pump_off'::public.event_type END,
    ...
    IF v_fail_pending THEN
      ...
      SET status = 'error',
          error_message = COALESCE(error_message, 'RX divergente do payload esperado após 120s de reforço'),
          responded_at = COALESCE(responded_at, now())
      WHERE id = v_eq.pending_command_id
        AND status IN ('pending', 'sent')
        AND sent_at IS NOT NULL
        AND sent_at < now() - interval '120 seconds';
...
-- Reset comandos manuais recentemente marcados incorretamente como error com sent_at NULL,
-- para que o Electron possa pegá-los novamente. Só afeta comandos dos últimos 5 minutos.
UPDATE public.commands
SET status = 'pending',
    error_message = NULL,
    responded_at = NULL
WHERE type = 'manual'
  AND status = 'error'
  AND sent_at IS NULL
  AND error_message ILIKE '%divergente%'
