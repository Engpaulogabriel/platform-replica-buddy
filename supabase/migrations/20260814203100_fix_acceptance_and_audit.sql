-- ============================================================================
-- CORREÇÃO de 20260814203000 (já aplicada) — forward-only, somente leitura.
-- ----------------------------------------------------------------------------
-- Dois defeitos reais na versão anterior:
--
--  1) automation_report_acceptance() devolvia o critério 4 DUAS vezes: a segunda
--     cláusula NOT EXISTS não incluía '4. duplicidade sem transição física' na
--     união interna, então a linha "PASSOU 0" era emitida mesmo havendo
--     violações. Reescrito com lista de critérios + LEFT JOIN: cada critério
--     passa a emitir EXATAMENTE UMA linha, por construção — não por acaso.
--
--  2) automation_report_farm_audit() usava count(*) FILTER sobre um LEFT JOIN.
--     Fazenda sem nenhum evento contava a linha NULA do join e aparecia com
--     1 "transição sem prova". Trocado por count(o.id) FILTER, que ignora a
--     linha artificial. Toda contagem é zero real.
--
-- Nada é alterado no dado. Idempotente.
-- ============================================================================
...
SELECT f.name,
  -- count(o.id): ignora a linha NULA que o LEFT JOIN produz em fazenda vazia
  count(o.id)::int,
...
-- LEFT JOIN da lista fixa com as violações: exatamente uma linha por critério,
-- exista ou não violação. Impossível duplicar.
SELECT k.c,
       count(v.id)::int,
