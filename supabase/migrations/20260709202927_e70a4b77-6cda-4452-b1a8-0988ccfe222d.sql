...
CREATE OR REPLACE FUNCTION public.is_free_demand_day(_date date, _farm_id uuid DEFAULT NULL)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT
    EXTRACT(dow FROM _date)::int IN (0, 6)
    OR EXISTS (SELECT 1 FROM public.national_holidays h WHERE h.holiday_date = _date);
$$;
REVOKE ALL ON FUNCTION public.is_free_demand_day(date, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_free_demand_day(date, uuid) TO authenticated, service_role;

ALTER TABLE public.energy_efficiency_daily
  ADD COLUMN IF NOT EXISTS is_free_demand boolean NOT NULL DEFAULT false;

-- Drop antes de recriar com novo tipo de retorno
DROP FUNCTION IF EXISTS public.get_energy_efficiency_history(uuid, integer);
...
BEGIN
  ...
  SELECT
    ...
    ELSE ROUND(GREATEST(0::numeric, LEAST(100::numeric,
      100::numeric - (
        (COALESCE(m.post_lost_pump_min_v, 0) + COALESCE(m.gap_min_v, 0)
         + CASE WHEN is_free THEN 0 ELSE COALESCE(m.minutes_peak_v, 0) END)::numeric
    ...
    (COALESCE(m.post_lost_pump_min_v, 0) + COALESCE(m.gap_min_v, 0)
     + CASE WHEN is_free THEN 0 ELSE COALESCE(m.minutes_peak_v, 0) END)::int AS lost_minutes,
...
BEGIN
  ...
  VALUES (_farm_id, r.cycle_date, r.pre_peak_shutdown_time, r.post_peak_startup_time,
          ...
          r.pumps_operated, r.minutes_on_during_peak, r.pre_peak_ok_count, r.post_peak_ok_count,
          is_free, now())
...
CREATE FUNCTION public.get_energy_efficiency_history(_farm_id uuid, _days integer DEFAULT 30)
 RETURNS TABLE(cycle_date date, efficiency_percent numeric, pumps_operated integer, post_peak_startup_time timestamp with time zone, pre_peak_shutdown_time timestamp with time zone, lost_minutes integer, pumps_on_during_peak integer, minutes_on_during_peak integer, pre_peak_ok_count integer, post_peak_ok_count integer, is_free_demand boolean)
 LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
#variable_conflict use_column
DECLARE
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(eed.is_free_demand, public.is_free_demand_day(r.d, _farm_id))
  FROM range_dates r
...
BEGIN
  ...
  post_per_pump AS (
    SELECT ep.id AS equipment_id, fvo.first_on,
           CASE WHEN fvo.first_on IS NULL THEN 540
                ELSE LEAST(540, GREATEST(0, FLOOR(EXTRACT(epoch FROM (fvo.first_on - cycle_start))/60))::int)
           END AS late_min,
           (fvo.first_on IS NULL) AS not_started
      FROM pump_equipment ep
      LEFT JOIN first_valid_on fvo ON fvo.equipment_id = ep.id
  ),
...
BEGIN
  FOR f IN SELECT id, COALESCE(timezone,'America/Sao_Paulo') AS tz FROM public.farms LOOP
    today_local := (now() AT TIME ZONE f.tz)::date;
    FOR d IN SELECT generate_series(today_local - 30, today_local - 1, interval '1 day')::date LOOP
      BEGIN
        PERFORM public.compute_energy_efficiency(f.id, d);
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'compute_energy_efficiency failed for farm % date %: %', f.id, d, SQLERRM;
      END;
    END LOOP;
  END LOOP;
END $$;
