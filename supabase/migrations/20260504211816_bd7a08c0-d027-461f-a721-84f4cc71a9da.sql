...
BEGIN
  ...
  LOOP
    ...
    -- desired_running só é alterado por eventos explícitos:
    -- 1) confirmação de comando manual (remote)
    -- 2) acionamento local válido (origin=local detectado)
    -- 3) janela de start de comando manual (mantém o desejado)
    -- 4) reset protetivo aguardando (mantém false)
    -- Nunca espelhar o RX por fallback — isso causava re-ligar após safety.
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
        ...
        v_blocked_until := now() + interval '30 seconds';
      ELSIF v_origin = 'local' THEN
        -- Acionamento local válido (origin foi explicitamente detectado como local
        -- E não está dentro de janela de safety/comando ativo).
        v_next_desired_running := v_received_bit = '1';
      ELSE
        -- Fallback: NÃO altera desired_running. Apenas reflete estado real
        -- em last_outputs_state. Evita re-ligar bomba após safety expirar
        -- enquanto a PLC ainda responde {1} por falha de comunicação.
        v_next_desired_running := NULL;
      END IF;
    END IF;
