...
ALTER TABLE public.automation_schedules
  ADD COLUMN IF NOT EXISTS last_on_executed_at timestamptz,
  ADD COLUMN IF NOT EXISTS last_off_executed_at timestamptz;
...
DECLARE
  ...
  v_holiday_mmdd text;
  v_now_min int;       -- minutes since 00:00 local
  v_on_min int;
  v_off_min int;
  ...
  WINDOW_MIN constant int := 2; -- ±/+ window in minutes (current >= scheduled AND current < scheduled + 2)
BEGIN
  ...
  LOOP
    ...
    -- Holidays: same behavior as before (free-demand skips, special-schedule overrides)
    ...
    -- Parse scheduled times into minutes
    v_on_min := NULL;
    v_off_min := NULL;
    ...
    -- Already-executed-today check (in farm tz)
    ...
    THEN
      ...
      UPDATE public.automation_schedules
        SET last_on_executed_at = v_now
        WHERE id = v_sched.id;

      v_enqueued := v_enqueued + 1;

    ELSIF v_fire_on AND v_currently_running = true THEN
      -- already in desired state: mark as executed to prevent re-checking
      UPDATE public.automation_schedules
        SET last_on_executed_at = v_now
        WHERE id = v_sched.id;
    END IF;
    ...
    THEN
      ...
      UPDATE public.automation_schedules
        SET last_off_executed_at = v_now
        WHERE id = v_sched.id;
  ...
  VALUES (v_evaluated, v_enqueued, jsonb_build_object('mode', 'single-fire-2min-window', 'version', 3));
