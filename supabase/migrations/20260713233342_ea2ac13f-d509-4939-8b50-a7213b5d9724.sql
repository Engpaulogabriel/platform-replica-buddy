...
BEGIN
  ...
  LOOP
    ...
    -- ─────────────────────────────────────────────────────────────────────
    -- REGRA (fix 2026-07-13): desired_running representa a INTENÇÃO do
    -- operador/automação. NUNCA é sobrescrita por sincronização com o estado
    -- físico real lido pelo rádio. Só muda:
    --   • quando um comando manual/automação foi enfileirado (trigger
    --     sync_equipment_pending_command_on_insert);
    --   • quando o timer de segurança expira (reset protetivo → false);
    --   • quando um comando pendente é CONFIRMADO pela telemetria (mantém
    --     a intenção original, idempotente).
    -- Nos demais ramos (bomba desobediente, RX espontâneo, acionamento local)
    -- v_next_desired_running fica NULL → COALESCE preserva o desired atual.
    -- ─────────────────────────────────────────────────────────────────────
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit IS NOT NULL
        ...
        -- Bomba não obedeceu: MANTÉM a intenção do operador (desired inalterado).
        v_next_desired_running := NULL;
        v_clear_pending := true;
      ...
      ELSIF COALESCE(v_eq.last_actuation_origin, '') = 'local' AND NOT v_recent_safety_expiry THEN
        -- Acionamento local: NÃO altera desired_running — operador remoto
        -- continua sendo dono da intenção. Só reflete estado real em
        -- last_outputs_state e mantém janela de bloqueio.
        v_next_desired_running := NULL;
        v_blocked_until := COALESCE(v_eq.command_blocked_until, now() + interval '30 seconds');
      ELSE
        -- RX espontâneo genérico: NÃO sincroniza desired com estado real.
        v_next_desired_running := NULL;
      END IF;
    END IF;
