...
BEGIN
  ...
  WITH sensors AS (
    ...
    WHERE farm_id = _farm_id
      AND active = true
      AND type = 'nivel'
      AND level_last_raw_at >= now() - interval '15 minutes'
  ),
  recent AS (
  ...
  anchor AS (
    ...
    WHERE lh.read_at <= s.level_last_raw_at - interval '10 minutes'
      AND lh.read_at >= s.level_last_raw_at - interval '120 minutes'
