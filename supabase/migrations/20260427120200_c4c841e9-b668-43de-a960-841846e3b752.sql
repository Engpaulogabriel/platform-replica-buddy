-- Corrige apply_pump_telemetry: alerta de "local" só deve ser limpo quando
-- um comando MANUAL do usuário for obedecido. TX 0 automáticos de proteção
-- (backend-reset:local_*) NÃO devem voltar a marcar como 'remote'.
  ...
  LOOP
    ...
    IF v_state_changed THEN
      IF v_pending_confirms_expected AND NOT v_pending_is_protective_reset THEN
        -- Apenas comandos MANUAIS do usuário (não auto-resets de proteção) limpam o alerta local.
        v_origin := 'remote';
      ...
      END IF;
    ELSE
      -- Sem mudança de estado nesta leitura
      IF v_pending_confirms_expected AND NOT v_pending_is_protective_reset THEN
        v_origin := 'remote';
      END IF;

      -- Cenário: comando de LIGAR foi enviado, mas bomba continua desligada após janela de 180s.
      ...
      -- Cenário: bomba LIGADA localmente (recebeu '1' sem comando remoto pedindo).
      IF v_received_bit = '1'
         AND COALESCE(v_eq.desired_running, false) = false
         AND v_eq.pending_command_id IS NULL
         AND COALESCE(v_eq.last_actuation_origin, '') <> 'local' THEN
        v_origin := 'local';
        v_blocked_until := now() + interval '30 seconds';
      END IF;
    ...
    IF v_received_bit IN ('0', '1') THEN
      ...
      ELSIF v_received_bit = '0' AND COALESCE(v_eq.desired_running, false) = true
            AND (v_pending_started_at IS NULL OR v_pending_started_at <= now() - interval '180 seconds') THEN
        v_next_desired_running := false;
      ELSE
    ...
    -- Detecção de ligamento local não solicitado: força TX 0
    IF v_received_bit = '1'
       AND COALESCE(v_eq.desired_running, false) = false
       AND v_eq.pending_command_id IS NULL THEN
      IF NOT EXISTS (
        SELECT 1
        FROM public.commands c
