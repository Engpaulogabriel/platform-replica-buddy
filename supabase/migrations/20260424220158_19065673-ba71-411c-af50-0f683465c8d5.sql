...
BEGIN
  ...
  LOOP
    ...
    IF v_should_force_off THEN
      -- Janela anti-duplicata reduzida de 90s para 15s: precisamos reagir
      -- rapido se a bomba ligar/desligar local em sequencia
      IF NOT EXISTS (
        SELECT 1
        FROM public.commands c
...
BEGIN
  ...
  -- Janela anti-duplicata reduzida de 90s para 15s
  IF EXISTS (
    SELECT 1
    FROM public.commands c
