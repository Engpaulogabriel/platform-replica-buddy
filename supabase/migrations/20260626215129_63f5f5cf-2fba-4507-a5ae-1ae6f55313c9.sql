...
-- Offline threshold: 15 min for single-output equipments,
-- 20 min for multi-output PLCs (Boosters and similar).
-- Replaces the previous (telemetry_interval + 5) rule.
...
BEGIN
  ...
  -- Janela de offline: 15 min para equipamentos com 1 saída; 20 min para
  -- equipamentos com múltiplas saídas no mesmo PLC (Boosters e similares).
  ...
  ELSE
    SELECT COUNT(*) INTO _siblings
     ...
     WHERE plc_group_id = NEW.plc_group_id
       AND active = true;
  END IF;
  _offline_min := CASE WHEN _siblings > 1 THEN 20 ELSE 15 END;
  ...
  IF COALESCE(_alerts_enabled, false)
    ...
    -- Proteção anti-falso-positivo: só dispara se realmente está sem
    -- comunicação há mais do que a janela do equipamento.
  ...
  IF COALESCE(_alerts_enabled, false)
    ...
    -- Proteção anti-flap: só envia back_online se houve alerta de offline
    -- REAL enviado há mais de 5 minutos (e dentro das últimas 24 h).
    SELECT EXISTS (
      SELECT 1
