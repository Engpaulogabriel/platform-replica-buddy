...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Marca como LOCAL para sinalizar que o comando remoto não foi obedecido.
      ...
      -- Mesmo sem mudança detectada nesta leitura (1→1 repetido), garante o flag local.
      IF v_received_bit = '1'
      ...
      IF COALESCE(v_eq.desired_running, false) = true
        ...
        IF v_pending_started_at IS NULL OR v_pending_started_at <= now() - interval '180 seconds' THEN
          v_should_force_off := true;
          -- Bomba esperava estar ligada e está desligada → desligamento local
    ...
    -- Lógica de desired_running (igual à anterior)
    IF v_received_bit IN ('0', '1') THEN
