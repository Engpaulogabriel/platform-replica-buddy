-- v3.25.9: distinção "eco de comando remoto" vs "nova atuação local" por ESTADO DO
-- COMANDO, não por tempo. Substitui a janela de 30s (20260723130000).
--
-- Regra:
--   • Se NÃO existe comando do operador NÃO-confirmado para o equipamento (status
--     pending/sent/delivered) → um espontâneo que muda o estado é LOCAL, imediatamente.
--   • Se EXISTE comando não-confirmado (na FILA 'pending', em voo 'sent'/'delivered'):
--       - eco/confirmação (bit do comando em voo == bit recebido) → 'remote' via
--         v_pending_confirms_expected;
--       - caso contrário o local é SUPRIMIDO (v_pending_command_active gateia todos os
--         ramos locais) — inclui o caso de FILA (ligar N bombas em série: as que ainda
--         não receberam TX têm comando 'pending' e não podem ser marcadas local).
--
-- Mudanças vs 20260714051458:
--   1. Removido o bloco de janela temporal do v_recent_remote_match (fica sempre false).
--   2. v_pending_command_active passa a incluir 'delivered' (além de pending/sent).
--   3. Removido "AND NOT v_recent_remote_match" das condições dos ramos locais.
-- Restante idêntico.
