...
BEGIN
  ...
  LOOP
    ...
    -- v3.15: janela mais ampla (90s) para decidir se um acionamento é "local puro"
    -- (sem qualquer comando manual OU de automação recente). Se não houver
    -- comando remoto nos últimos 90s e o Electron/telemetria indicar mudança
    -- de estado, é uma atuação local — NÃO devemos alterar desired_running
    -- (para não gerar alertas falsos de "operador desligou pela Plataforma Web").
    SELECT EXISTS (
      SELECT 1 FROM public.commands c
      WHERE c.farm_id = _farm_id
        AND c.equipment_id = v_eq.id
    ...
    -- ─── Local puro (v3.15): mudança de estado OU origem explícita 'local'
    -- E nenhum comando manual/automação nos últimos 90s → é acionamento local.
    -- Nesse caso NÃO devemos mexer em desired_running.
    ...
    IF v_received_bit IN ('0', '1') THEN
      IF v_local_no_recent_cmd THEN
        -- ATUAÇÃO LOCAL SEM COMANDO RECENTE (v3.15):
        -- preserva desired_running (NÃO altera o "estado desejado" do operador).
        -- Isso evita que o frontend interprete a mudança como comando remoto
        -- e dispare alertas falsos de "DESLIGADO / Via: Plataforma Web".
        v_next_desired_running := NULL;
        v_clear_pending := false;
        v_fail_pending := false;
        v_enqueue_safety_off := false;
