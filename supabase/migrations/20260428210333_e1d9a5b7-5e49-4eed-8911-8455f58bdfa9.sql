...
BEGIN
  ...
  LOOP
    ...
    -- Fallback crítico: mesmo que pending_command_id tenha sido limpo cedo demais,
    -- ainda protege o comando remoto recente (LIGAR ou DESLIGAR) contra eco
    -- contraditório nos primeiros 60s.
    ...
    IF v_pending_is_manual AND v_payload_safe IS NOT NULL AND v_pending_frame IS NOT NULL THEN
      ...
      -- Janela de início (60s) — tolera eco contraditório em AMBAS as direções:
      --   • LIGAR : esperado=1, recebido=0  (bomba ainda não ligou)
      --   • DESLIGAR: esperado=0, recebido=1  (bomba ainda não desligou)
    ...
    ELSE
      ...
      -- Eco contraditório (esperava 1, veio 0) FORA da janela de 60s → trata como local.
      ...
      -- Espelho: eco contraditório (esperava 0, veio 1) FORA da janela de 60s
      -- também é tratado como acionamento local. Dentro da janela: aguarda
      -- o espontâneo final (ainda em VERIFYING_OFF).
      IF v_pending_is_manual
    ...
    IF v_received_bit IN ('0', '1') THEN
      IF v_pending_within_start_window THEN
        -- Mantém o desejado conforme o que o usuário pediu (1=ligar, 0=desligar)
        -- até o eco final confirmar. Não limpa nem falha o pending.
      ...
      ELSIF v_pending_is_manual AND v_pending_expected_bit = '0' AND v_received_bit = '1' THEN
        -- Espelho fora da janela: comando de DESLIGAR não foi obedecido em 60s.
        v_next_desired_running := false;
        v_clear_pending := true;
        v_fail_pending := COALESCE(v_pending_status, 'pending') <> 'executed';
    ...
    ELSIF v_fail_pending AND v_eq.pending_command_id IS NOT NULL THEN
      ...
      SET status = 'error',
          ...
          response = COALESCE(_raw_response, response),
          error_message = CASE
            WHEN v_pending_expected_bit = '0' THEN 'Telemetria confirmou 1: comando de desligar nao foi aplicado'
            ELSE 'Telemetria confirmou 0: comando de ligar nao foi aplicado'
          END
      WHERE id = v_eq.pending_command_id
        AND farm_id = _farm_id
    ...
    -- Detecção de "ligada localmente sem comando" — agora respeita a janela
    -- de início para não disparar reset enquanto o desligamento ainda está
    -- sendo aguardado (esperado=0, recebido=1 nos primeiros 60s).
    IF v_received_bit = '1'
       AND COALESCE(v_eq.desired_running, false) = false
       AND v_eq.pending_command_id IS NULL
       AND NOT v_pending_within_start_window THEN
      IF NOT EXISTS (
        ...
        WHERE c.farm_id = _farm_id
          AND c.equipment_id = v_eq.id
          AND c.source_device IN ('backend-reset:local_startup_detected', 'backend-reset:local_shutdown_detected')
          AND c.status IN ('pending', 'sent')
          AND c.created_at > now() - interval '15 seconds'
      ) THEN
        PERFORM public.enqueue_reset_pump_command(_farm_id, v_eq.id, 'local_startup_detected');

        INSERT INTO public.agent_logs (farm_id, level, category, message)
        VALUES (
          _farm_id, 'warn', 'safety',
          format('Bomba %s (%s) foi LIGADA LOCALMENTE sem comando remoto — TX 0 enfileirado para desligar.', v_eq.id, v_eq.name)
        );
      END IF;
    END IF;
    ...
    IF v_should_force_off THEN
      IF NOT EXISTS (
        ...
        VALUES (
          _farm_id, 'warn', 'safety',
          format('Bomba %s (%s) nao obedeceu o comando de ligar/desligou localmente — TX 0 enfileirado para travar rele.', v_eq.id, v_eq.name)
        );
      END IF;
    END IF;
  END LOOP;
