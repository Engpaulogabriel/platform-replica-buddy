...
DECLARE
  ...
  v_recent_manual_within_grace boolean := false;
...
BEGIN
  ...
  -- Janela de carencia: se houve comando manual nos ultimos 15s, NAO marca como local
  -- mesmo que a leitura divirja (PLC ainda esta processando)
  SELECT EXISTS (
    SELECT 1 FROM public.commands c
    WHERE c.equipment_id = v_eq_id
      AND c.type = 'manual'
      AND COALESCE(c.sent_at, c.created_at) > now() - interval '15 seconds'
  ) INTO v_recent_manual_within_grace;
  ...
  IF v_state_changed THEN
    ...
    ELSIF v_command_is_manual THEN
      ...
      IF v_recent_manual_expected_known AND v_recent_manual_expected_running = v_new_running THEN
        v_origin := 'remote';
      ELSIF v_recent_manual_within_grace THEN
        -- Dentro da carencia, ignora divergencia (nao marca como local)
        v_state_changed := false;
        v_payload_safe := NULL; -- nao sobrescreve last_outputs_state
      ELSE
        v_origin := 'local';
    ...
    ELSE
      ...
      IF v_recent_manual_expected_known AND v_recent_manual_expected_running = v_new_running THEN
        v_origin := 'remote';
      ELSIF v_recent_manual_within_grace THEN
        -- Polling leu durante carencia: ignora divergencia
        v_state_changed := false;
        v_payload_safe := NULL;
      ELSE
        v_origin := 'local';
...
BEGIN
  ...
  LOOP
    -- Pula polling se houve comando manual nos ultimos 15s (carencia para PLC efetivar)
    SELECT EXISTS (
      SELECT 1 FROM public.commands c
      WHERE c.equipment_id = v_eq.id
        AND c.type = 'manual'
        AND COALESCE(c.sent_at, c.created_at) > now() - interval '15 seconds'
    ) INTO v_recent_manual;

    IF v_recent_manual THEN
      CONTINUE;
    END IF;
    ...
    v_read_only := (v_eq.last_actuation_origin = 'local')
                   OR (v_eq.command_blocked_until IS NOT NULL AND v_eq.command_blocked_until > now());

    IF v_read_only THEN
      v_frame := '[' || v_tsnn || '_1_]{}[' || v_tsnn || '_ETX_]' || E'\r';
    ELSE
