...
BEGIN
  ...
  -- Confere se o pending manual foi atendido pelo payload recebido,
  -- INDEPENDENTE de ter havido mudança de estado (o operador pode mandar
  -- desligar uma bomba ja desligada e o RX confirma com 0 -- isso ainda
  -- e' "comando atendido" e deve limpar o pending imediatamente).
  IF v_eq_pending IS NOT NULL THEN
    ...
    IF v_pending_is_manual AND v_payload_safe IS NOT NULL AND v_pending_frame IS NOT NULL THEN
      ...
      v_received_bit := substring(v_payload_safe from v_eq_saida::int for 1);
      IF v_pending_payload ~ '^[01]$' THEN
        v_pending_expected_bit := v_pending_payload;
      ELSIF v_pending_payload ~ '^[01]{6}$' THEN
        v_pending_expected_bit := substring(v_pending_payload from v_eq_saida::int for 1);
      END IF;
  ...
  -- Limpa pending sempre que o RX confirma a intencao do manual (mesmo
  -- sem mudanca de estado), ou quando e' a resposta direta do polling.
  v_clear_pending := v_pending_confirms_expected
                    OR (
                      _command_id IS NOT NULL
                      AND _command_id = v_eq_pending
  ...
  -- Se houve confirmacao do pending manual, marca o comando como executado
  -- imediatamente (alem do polling/RX direto que ja era tratado abaixo).
  IF v_pending_confirms_expected AND v_eq_pending IS NOT NULL THEN
