-- Reaplica regra definitiva: offline 15/20 min + anti-flap 5 min antes de qualquer WhatsApp.
...
BEGIN
  ...
  IF COALESCE(_alerts_enabled, false)
    ...
    -- Proteção anti-flap: offline+online ou online+offline em menos de 5 min
    -- não gera nenhum novo alerta WhatsApp.
  ...
  IF COALESCE(_alerts_enabled, false)
    ...
    -- Back-online só é real se houve alerta offline enviado há mais de 5 min.
    SELECT EXISTS (
      SELECT 1
      FROM public.whatsapp_alerts_log
      WHERE equipment_id = NEW.id
