...
BEGIN
  ...
  LOOP
    ...
    -- v3.14.9: Detecta comando manual recente (últimos 30s) para esse equipamento.
    -- Se existiu um comando remoto recente, o estado real divergente NÃO é
    -- considerado local imediatamente — dá tempo pra bomba obedecer.
    SELECT EXISTS (
      SELECT 1 FROM public.commands c
      WHERE c.farm_id = _farm_id
        AND c.equipment_id = v_eq.id
    ...
    -- ─── Determinação de origem (v3.14.9) ────────────────────────────────
    -- Regra nova: se o estado real DIVERGE do desired_running e NÃO há
    -- comando manual/pendente recente que justifique, marca como LOCAL —
    -- mesmo que o estado não tenha mudado neste ciclo. Isso faz o badge
    -- LOCAL aparecer quando o operador manda desligar mas a bomba (em modo
    -- local, chave física) permanece ligada.
    ...
    ELSIF v_received_bit IN ('0','1')
      ...
      -- Divergência persistente: bomba não obedeceu ao desired do operador.
      v_origin := 'local';
      v_blocked_until := now() + interval '30 seconds';
    ELSIF COALESCE(v_eq.last_actuation_origin, '') = 'local'
          AND v_received_bit IN ('0','1')
          AND v_desired_matches_received THEN
      -- Estado voltou a bater com o desired → limpa marcação local.
      v_origin := 'remote';
