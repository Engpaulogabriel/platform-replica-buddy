...
BEGIN
  ...
  IF _turning_on THEN
    SELECT c.id INTO r
     ...
     LIMIT 1;
    IF FOUND THEN
      RETURN QUERY SELECT 'auto'::public.event_origin, 'Modo Automático'::text,
...
BEGIN
  ...
  IF v_intent = 'turn_on' THEN
    SELECT c.id INTO r
     ...
     ORDER BY abs(extract(epoch FROM (c.created_at - al.occurred_at))) LIMIT 1;
    IF FOUND THEN
      RETURN QUERY SELECT 'auto', NULL::uuid, NULL::text, 'Modo Automático'::text,
...
-- Correção retroativa dos 4 acionamentos do Modo Automático (Sossego, 04/09 21:15-21:19)
UPDATE public.automation_log al
SET origin = 'auto'::public.event_origin,
    actor_label = 'Modo Automático',
    details = COALESCE(al.details,'{}'::jsonb)
              || jsonb_build_object(
                   'authorship_source','cloud_automation',
                   'command_id', c.id,
                   'corrected_from','local',
                   'correction_note','classificado como Modo Automático (cloud-automation)')
FROM public.commands c
WHERE al.id IN (
  'b0148b6f-2fe6-4da9-b529-18eb13e436d1',
  '9cb18a99-c6a4-4561-8cf6-df24c67e5ef1',
  '8c0fea8a-d274-4932-9ac4-b4ea290950b7',
  'eebdc1d5-c880-4c14-87e0-2437d64474db'
)
