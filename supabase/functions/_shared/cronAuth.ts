...
export interface CronAuthEnv {
  ...
  /** Segredo adicional aceito durante rotação atômica (nunca substitui o antigo
   *  antes de o novo estar válido nos dois lados). */
  cronSecretAlt?: string | null;
...
export function isAuthorizedCron(headers: Headers, env: CronAuthEnv): CronAuthResult {
  ...
  const secretAlt = (env.cronSecretAlt ?? "").trim();
  ...
  if (!secret && !secretAlt) {
    return { ok: false, status: 403, reason: "cron_secret_not_configured" };
  }

  const provided = (headers.get("x-cron-secret") ?? "").trim();
  // Aceita o segredo vigente OU o segredo de rotação. Enquanto os dois valem,
  // não existe janela sem CRON_SECRET válido.
  if (provided && secret && timingSafeEqual(provided, secret)) return { ok: true, via: "cron_secret" };
  if (provided && secretAlt && timingSafeEqual(provided, secretAlt)) return { ok: true, via: "cron_secret" };
...
export function guardCron(req: Request, cors: Record<string, string> = {}): Response | null {
  const res = isAuthorizedCron(req.headers, {
    ...
    // deno-lint-ignore no-explicit-any
    cronSecretAlt: (globalThis as any).Deno?.env?.get("CRON_SECRET_V2"),
    // deno-lint-ignore no-explicit-any
