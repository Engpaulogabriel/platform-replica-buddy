...
const DAILY_REMINDER_MS = 24 * 60 * 60 * 1000; // dedup: relembra no máximo 1x/24h

// Fazendas internas de bancada — nunca geram alerta de watchdog.
// Match por substring: o nome pode vir como "Laboratório RENOV" ou "Fazenda Laboratório RENOV".
const SKIP_FARM_PATTERNS = ["laborat"];
const isSkippedFarm = (name?: string | null) => {
  if (!name) return false;
  const n = String(name).trim().toLowerCase();
  return SKIP_FARM_PATTERNS.some((p) => n.includes(p));
};
...
Deno.serve(async (req) => {
  ...
  // Dedup: só reenvia um alerta ativo se já passaram 24h desde o último envio.
  const shouldSend = (map: Map<string, string>, farmId: string) => {
    if (!map.has(farmId)) return true;
    const last = map.get(farmId);
    return !last || (nowMs - new Date(last).getTime()) >= DAILY_REMINDER_MS;
  };

  for (const r of (sh ?? []) as any[]) {
    if (r?.farm?.is_demo || !r.last_heartbeat) continue;
    const farmName = r?.farm?.name ?? "Fazenda";
    if (isSkippedFarm(farmName)) continue;
    ...
    if (bridgeDead && shouldSend(activeBridge, r.farm_id)) {
    ...
    } else if (!bridgeDead && agentOnline && activeBridge.has(r.farm_id)) {
  ...
  for (const r of (sh ?? []) as any[]) {
    ...
    const offline = ageMs > AGENT_OFFLINE_MS;
    if (offline && shouldSend(activeAgent, r.farm_id)) {
      const res = await invokeAlert({
        type: "alert", immediate: true, source: "agent_offline_watchdog",
    ...
    } else if (!offline && activeAgent.has(r.farm_id)) {
  ...
  for (const e of (eqs ?? []) as any[]) {
    ...
    if (isSkippedFarm(e?.farm?.name)) continue;
  ...
  for (const [farmId, g] of staleByFarm) {
    if (g.names.length >= COM_MISSING_MIN_COUNT) {
      ...
      if (!shouldSend(activeCom, farmId)) continue;
      const res = await invokeAlert({
        type: "alert", immediate: true, source: "agent_offline_watchdog",
