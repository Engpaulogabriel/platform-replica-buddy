...
Deno.serve(async (req) => {
  ...
  // 5) URL assinada do main.enc daquela versão (1h)
  let encUrl: string | null = null;
  const { data: signed } = await supabase.storage
    .from("agent-releases")
    .createSignedUrl(`${version}/main.enc`, 3600);
  encUrl = signed?.signedUrl ?? null;

  return json({
    ok: true,
    aes_key: keyRow.aes_key,
    algo: keyRow.algo ?? "aes-256-gcm",
    version,
    enc_url: encUrl,
    enc_expires_in: encUrl ? 3600 : null,
  });
});
